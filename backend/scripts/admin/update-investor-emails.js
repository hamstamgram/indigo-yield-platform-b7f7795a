...
function identifyPlaceholderEmails(investors) {
  ...
  return investors.filter(investor => {
    ...
    return placeholderDomains.some(domain => email.includes(domain)) ||
           email === '' ||
...
async function updateInvestorEmails() {
  ...
  try {
    ...
    const selectedAction = await promptSelection('Select action:', actions, (action) => action.label);
      ...
      case 'bulk':
        ...
        if (bulkResult.updates.length > 0) {
          ...
          bulkResult.updates.forEach(update => {
            ...
            const reason = update.success ? `${update.oldEmail} → ${update.newEmail}` : update.reason;
