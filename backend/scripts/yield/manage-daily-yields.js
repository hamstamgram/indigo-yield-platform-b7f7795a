...
async function applyDailyYields() {
  ...
  const date = await prompt(`Apply yields for date (YYYY-MM-DD) [${getCurrentDate()}]: `) || getCurrentDate();
...
async function viewYieldHistory() {
  ...
  for (const [date, assets] of Object.entries(yieldsByDate)) {
    ...
    console.log(`${date} | ${btc.padEnd(6)} | ${eth.padEnd(6)} | ${usdt.padEnd(6)} | ${sol.padEnd(6)} | ${usdc.padEnd(6)} | ${eurc.padEnd(6)}`);
  }
}
