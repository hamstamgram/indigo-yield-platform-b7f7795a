              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-mono text-blue-600">{asset.currentRate?.toFixed(4)}% APY</span>
                  </div>

                  <div className="flex justify-between text-sm">
                    <span>Daily Yield:</span>
                    <span className="font-mono text-green-600">
                      +{formatTokenBalance(asset.dailyYield || 0, asset.symbol)}
                    </span>
                  </div>

                  <div className="flex justify-between text-sm border-t pt-1 mt-2">
                    <span className="font-medium">Total Earned:</span>
                    <span className="font-mono font-bold text-green-600">
                      +{formatTokenBalance(asset.totalEarned || 0, asset.symbol)}
                    </span>
                  </div>

                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>Yield:</span>
                    <span className="font-mono">{asset.yieldPercentage?.toFixed(2)}%</span>
                  </div>
                </div>
              </div>
            ))}
