...
export async function getHistoricalReports(filters?: {
  ...
  try {
    ...
    return (data || []).map((r: PerformanceRow) => ({
      ...
      report_month: r.period?.period_end_date ?? undefined,
      asset_code: r.fund_name,
      opening_balance: r.mtd_beginning_balance ?? undefined,
      closing_balance: r.mtd_ending_balance ?? undefined,
      additions: r.mtd_additions ?? undefined,
      withdrawals: r.mtd_redemptions ?? undefined,
      yield_earned: r.mtd_net_income ?? undefined,
