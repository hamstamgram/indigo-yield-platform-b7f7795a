...
export async function generatePeriodSnapshot(
  ...
  try {
    const { data, error } = await supabase.rpc("generate_fund_period_snapshot", {
      p_fund_id: fundId,
...
export async function lockPeriodSnapshot(
  ...
  try {
    const { data, error } = await supabase.rpc("lock_fund_period_snapshot", {
      p_fund_id: fundId,
      p_period_id: periodId,
      p_admin_id: adminId,
    });
...
export async function getPeriodOwnership(
  ...
  try {
    const { data, error } = await supabase.rpc("get_period_ownership", {
      p_fund_id: fundId,
      p_period_id: periodId,
    });
