...
import { PerformanceDataEditor } from "@/features/admin/reports/components/PerformanceDataEditor";
...
import { deletePerformanceReport } from "@/services/admin/reportQueryService";
...
const InvestorReports = () => {
  ...
  const { data, isLoading: loading, refetch } = useAdminInvestorReports(selectedMonth);
  ...
  const handleDeleteReport = async () => {
    ...
    setDeleting(true);
    try {
      await deletePerformanceReport(reportToDelete.id);
      toast.success(`Report for ${reportToDelete.name} deleted successfully`);
      refetch();
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      ...
      <Alert className="bg-primary/5 border-primary/20">
        ...
        <AlertDescription className="flex items-center justify-between">
          <span>
            Report email sending has been moved to the Report Delivery Center for better tracking
            and reliability.
          </span>
      ...
      <div className="space-y-6">
                    ...
                    <TableRow key={report.investor_id}>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          {report.has_reports && report.assets[0]?.report_id && (
                            <Button
                              size="sm"
                              variant="outline"
                              ...
                              onClick={() => {
                                setReportToDelete({
                                  id: report.assets[0].report_id!,
                                  name: report.investor_name,
                                });
                                setDeleteDialogOpen(false); // Close other potential dialogs
                                setTimeout(() => setDeleteDialogOpen(true), 10);
                              }}
                              title="Delete Performance Report"
                            >
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Delete Performance Report?</DialogTitle>
              <DialogDescription>
                Are you sure you want to delete the performance report for{" "}
                <span className="font-bold text-foreground">{reportToDelete?.name}</span>? This
                action cannot be undone and will remove the data from statements.
              </DialogDescription>
            </DialogHeader>
