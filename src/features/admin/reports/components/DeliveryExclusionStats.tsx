    ...
    <Card className="border-blue-200 dark:border-blue-900">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant={
              deliveryRate === 100 ? "default" : deliveryRate > 50 ? "secondary" : "destructive"
            }
          >
      ...
      <CardContent className="space-y-4">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <XCircle
                      className={`h-4 w-4 ${breakdown.failed > 0 ? "text-destructive" : "text-muted-foreground"}`}
                    />
                    <span className="text-xs font-medium text-muted-foreground">Failed</span>
                  </div>
                  <p
                    className={`text-xl font-bold ${breakdown.failed > 0 ? "text-destructive" : "text-muted-foreground"}`}
                  >
                    {breakdown.failed}
                  </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              {breakdown.noStatement > 0 && (
                <div className="flex items-center gap-3 p-2 rounded bg-muted/50 border">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{breakdown.noStatement} no statement</p>
                    <p className="text-xs text-muted-foreground truncate">
              ...
              {breakdown.cancelled > 0 && (
                <div className="flex items-center gap-3 p-2 rounded bg-muted/50 border">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{breakdown.cancelled} cancelled</p>
                    <p className="text-xs text-muted-foreground truncate">Manually cancelled</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Generate statements for this period first before sending emails. Go to Investor
                Reports → Generate Statements.
              </p>
            </div>
          </div>
        )}
        ...
        {breakdown.statementsGenerated > 0 &&
          breakdown.alreadySent === breakdown.statementsGenerated && (
