...
interface BulkUnvoidDialogProps {
  open: boolean;
...
export function BulkUnvoidDialog({
  open,
  onOpenChange,
  transactions,
  summary,
      ...
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Restore {summary.count} Voided Transaction{summary.count !== 1 ? "s" : ""}
          </DialogTitle>
          <DialogDescription>
            This will restore all selected voided transactions. Positions will be updated.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <Alert className="border-amber-500/50 bg-amber-500/10">
            <div className="flex items-start gap-2">
              ...
              <AlertDescription className="text-amber-700 text-sm">
                AUM records may need recalculation after restoring transactions. Run AUM
                recalculation for affected funds and dates if needed.
              </AlertDescription>
            </div>
          </Alert>
                    ...
                    {transactions.map((tx) => (
                      <tr key={tx.id} className="border-t">
                        ...
                        <td className="px-3 py-1 text-right font-mono">
                          {formatAssetValue(tx.amount, tx.asset)} {tx.asset}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="space-y-2">
            <Label htmlFor="bulk-unvoid-reason">Reason for restoring *</Label>
            <Textarea
              id="bulk-unvoid-reason"
              placeholder="Explain why these transactions are being restored..."
              value={reason}
