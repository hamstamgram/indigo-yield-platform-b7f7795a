    ...
    <Card className={hasRoutings ? "border-orange-500/30 bg-orange-500/5" : ""}>
      <CardHeader className="pb-3">
        <div className="flex items-center gap-3">
          <ArrowRightLeft
            className={`h-6 w-6 ${hasRoutings ? "text-orange-500" : "text-muted-foreground"}`}
          />
          <div>
            ...
            <div className="space-y-1">
              {Object.entries(summary.byAsset).map(([asset, data]) => (
                <div key={asset} className="flex items-center gap-2">
                  ...
                  <p
                    className={`text-lg font-mono font-bold ${hasRoutings ? "text-orange-600" : "text-muted-foreground"}`}
                  >
