...
export async function generateStatementPDF(statementData: StatementData): Promise<Blob> {
  ...
  statementData.assets.forEach(asset => {
    asset.transactions.forEach(tx => {
      ...
      const amountText = tx.type === 'withdrawal' || tx.type === 'fee' 
        ? `-${formatCurrency(tx.amount)}` 
        : formatCurrency(tx.amount);
      
      pdf.text(amountText, pageWidth - margin - 50, yPosition, { align: 'right' });
      pdf.text(formatCurrency(tx.running_balance || 0), pageWidth - margin - 20, yPosition, { align: 'right' });
      yPosition += 5;
    });
  });
  ...
  pdf.text('This statement is confidential and proprietary to Indigo Yield Platform.', 
