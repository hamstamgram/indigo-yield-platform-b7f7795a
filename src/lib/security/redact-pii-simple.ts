...
export function redactObject<T extends Record<string, any>>(obj: T, options: RedactionOptions = {}): T {
...
export function redactPII<T>(data: T, options: RedactionOptions = {}): T {
  ...
  if (data && typeof data === 'object') return redactObject(data as Record<string, any>, options) as T;
  return data;
}

export const redactForLogging = <T>(data: T) => redactPII(data, { patterns: DEFAULT_PATTERNS });
export const redactForAdmin = <T>(data: T) => redactPII(data, { preserveLength: true, customMask: '•' });
export const redactForAudit = <T>(data: T) => redactPII(data, { keys: ['password', 'token', 'secret'] });