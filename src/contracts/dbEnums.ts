...
export const AccessEventSchema = z.enum(ACCESS_EVENT_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid access_event: "${ctx.data}". Valid: ${ACCESS_EVENT_VALUES.join(", ")}` };
    }
...
export const AccountTypeSchema = z.enum(ACCOUNT_TYPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid account_type: "${ctx.data}". Valid: ${ACCOUNT_TYPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const ApprovalOperationTypeSchema = z.enum(APPROVAL_OPERATION_TYPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid approval_operation_type: "${ctx.data}". Valid: ${APPROVAL_OPERATION_TYPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const AssetCodeSchema = z.enum(ASSET_CODE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid asset_code: "${ctx.data}". Valid: ${ASSET_CODE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const AumPurposeSchema = z.enum(AUM_PURPOSE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid aum_purpose: "${ctx.data}". Valid: ${AUM_PURPOSE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const BENCHMARK_TYPE_VALUES = [
  "BTC",
  "ETH",
  "STABLE",
  "CUSTOM",
...
export const BenchmarkTypeSchema = z.enum(BENCHMARK_TYPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid benchmark_type: "${ctx.data}". Valid: ${BENCHMARK_TYPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const DocumentTypeSchema = z.enum(DOCUMENT_TYPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid document_type: "${ctx.data}". Valid: ${DOCUMENT_TYPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const ErrorCategorySchema = z.enum(ERROR_CATEGORY_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid error_category: "${ctx.data}". Valid: ${ERROR_CATEGORY_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const FundStatusSchema = z.enum(FUND_STATUS_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid fund_status: "${ctx.data}". Valid: ${FUND_STATUS_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const NotificationPrioritySchema = z.enum(NOTIFICATION_PRIORITY_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid notification_priority: "${ctx.data}". Valid: ${NOTIFICATION_PRIORITY_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const NotificationTypeSchema = z.enum(NOTIFICATION_TYPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid notification_type: "${ctx.data}". Valid: ${NOTIFICATION_TYPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const PlatformErrorCodeSchema = z.enum(PLATFORM_ERROR_CODE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid platform_error_code: "${ctx.data}". Valid: ${PLATFORM_ERROR_CODE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const SHARE_SCOPE_VALUES = [
  "portfolio",
  "documents",
  "statement",
] as const;
...
export const ShareScopeSchema = z.enum(SHARE_SCOPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid share_scope: "${ctx.data}". Valid: ${SHARE_SCOPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const TicketCategorySchema = z.enum(TICKET_CATEGORY_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid ticket_category: "${ctx.data}". Valid: ${TICKET_CATEGORY_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const TicketPrioritySchema = z.enum(TICKET_PRIORITY_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid ticket_priority: "${ctx.data}". Valid: ${TICKET_PRIORITY_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const TicketStatusSchema = z.enum(TICKET_STATUS_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid ticket_status: "${ctx.data}". Valid: ${TICKET_STATUS_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const TransactionStatusSchema = z.enum(TRANSACTION_STATUS_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid transaction_status: "${ctx.data}". Valid: ${TRANSACTION_STATUS_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const VisibilityScopeSchema = z.enum(VISIBILITY_SCOPE_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid visibility_scope: "${ctx.data}". Valid: ${VISIBILITY_SCOPE_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const WithdrawalActionSchema = z.enum(WITHDRAWAL_ACTION_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid withdrawal_action: "${ctx.data}". Valid: ${WITHDRAWAL_ACTION_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const WithdrawalStatusSchema = z.enum(WITHDRAWAL_STATUS_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid withdrawal_status: "${ctx.data}". Valid: ${WITHDRAWAL_STATUS_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export const YieldDistributionStatusSchema = z.enum(YIELD_DISTRIBUTION_STATUS_VALUES, {
  errorMap: (issue, ctx) => {
    if (issue.code === "invalid_enum_value") {
      return { message: `Invalid yield_distribution_status: "${ctx.data}". Valid: ${YIELD_DISTRIBUTION_STATUS_VALUES.join(", ")}` };
    }
    return { message: ctx.defaultError };
  },
});
...
export function getDefaultSubtype(uiType: UITxType): string {
  switch (uiType) {
    ...
    case "DEPOSIT": return "top_up";
    case "WITHDRAWAL": return "redemption";
    case "FEE": return "fee_charge";
    case "YIELD":
    case "INTEREST": return "yield_credit";
...
const _access_eventCheck: AccessEvent extends SupabaseAccessEvent ? SupabaseAccessEvent extends AccessEvent ? true : false : false = true;
void _access_eventCheck;
type SupabaseAccountType = Database["public"]["Enums"]["account_type"];
const _account_typeCheck: AccountType extends SupabaseAccountType ? SupabaseAccountType extends AccountType ? true : false : false = true;
void _account_typeCheck;
type SupabaseAppRole = Database["public"]["Enums"]["app_role"];
const _app_roleCheck: AppRole extends SupabaseAppRole ? SupabaseAppRole extends AppRole ? true : false : false = true;
void _app_roleCheck;
type SupabaseApprovalOperationType = Database["public"]["Enums"]["approval_operation_type"];
const _approval_operation_typeCheck: ApprovalOperationType extends SupabaseApprovalOperationType ? SupabaseApprovalOperationType extends ApprovalOperationType ? true : false : false = true;
void _approval_operation_typeCheck;
type SupabaseAssetCode = Database["public"]["Enums"]["asset_code"];
const _asset_codeCheck: AssetCode extends SupabaseAssetCode ? SupabaseAssetCode extends AssetCode ? true : false : false = true;
void _asset_codeCheck;
type SupabaseAumPurpose = Database["public"]["Enums"]["aum_purpose"];
const _aum_purposeCheck: AumPurpose extends SupabaseAumPurpose ? SupabaseAumPurpose extends AumPurpose ? true : false : false = true;
void _aum_purposeCheck;
type SupabaseBenchmarkType = Database["public"]["Enums"]["benchmark_type"];
const _benchmark_typeCheck: BenchmarkType extends SupabaseBenchmarkType ? SupabaseBenchmarkType extends BenchmarkType ? true : false : false = true;
void _benchmark_typeCheck;
type SupabaseDocumentType = Database["public"]["Enums"]["document_type"];
const _document_typeCheck: DocumentType extends SupabaseDocumentType ? SupabaseDocumentType extends DocumentType ? true : false : false = true;
void _document_typeCheck;
type SupabaseErrorCategory = Database["public"]["Enums"]["error_category"];
const _error_categoryCheck: ErrorCategory extends SupabaseErrorCategory ? SupabaseErrorCategory extends ErrorCategory ? true : false : false = true;
void _error_categoryCheck;
type SupabaseFeeKind = Database["public"]["Enums"]["fee_kind"];
const _fee_kindCheck: FeeKind extends SupabaseFeeKind ? SupabaseFeeKind extends FeeKind ? true : false : false = true;
void _fee_kindCheck;
type SupabaseFundStatus = Database["public"]["Enums"]["fund_status"];
const _fund_statusCheck: FundStatus extends SupabaseFundStatus ? SupabaseFundStatus extends FundStatus ? true : false : false = true;
void _fund_statusCheck;
type SupabaseNotificationPriority = Database["public"]["Enums"]["notification_priority"];
const _notification_priorityCheck: NotificationPriority extends SupabaseNotificationPriority ? SupabaseNotificationPriority extends NotificationPriority ? true : false : false = true;
void _notification_priorityCheck;
type SupabaseNotificationType = Database["public"]["Enums"]["notification_type"];
const _notification_typeCheck: NotificationType extends SupabaseNotificationType ? SupabaseNotificationType extends NotificationType ? true : false : false = true;
void _notification_typeCheck;
type SupabasePlatformErrorCode = Database["public"]["Enums"]["platform_error_code"];
const _platform_error_codeCheck: PlatformErrorCode extends SupabasePlatformErrorCode ? SupabasePlatformErrorCode extends PlatformErrorCode ? true : false : false = true;
void _platform_error_codeCheck;
type SupabaseShareScope = Database["public"]["Enums"]["share_scope"];
const _share_scopeCheck: ShareScope extends SupabaseShareScope ? SupabaseShareScope extends ShareScope ? true : false : false = true;
void _share_scopeCheck;
type SupabaseTicketCategory = Database["public"]["Enums"]["ticket_category"];
const _ticket_categoryCheck: TicketCategory extends SupabaseTicketCategory ? SupabaseTicketCategory extends TicketCategory ? true : false : false = true;
void _ticket_categoryCheck;
type SupabaseTicketPriority = Database["public"]["Enums"]["ticket_priority"];
const _ticket_priorityCheck: TicketPriority extends SupabaseTicketPriority ? SupabaseTicketPriority extends TicketPriority ? true : false : false = true;
void _ticket_priorityCheck;
type SupabaseTicketStatus = Database["public"]["Enums"]["ticket_status"];
const _ticket_statusCheck: TicketStatus extends SupabaseTicketStatus ? SupabaseTicketStatus extends TicketStatus ? true : false : false = true;
...
const _tx_sourceCheck: TxSource extends SupabaseTxSource ? SupabaseTxSource extends TxSource ? true : false : false = true;
void _tx_sourceCheck;
type SupabaseTxType = Database["public"]["Enums"]["tx_type"];
const _tx_typeCheck: TxType extends SupabaseTxType ? SupabaseTxType extends TxType ? true : false : false = true;
void _tx_typeCheck;
type SupabaseVisibilityScope = Database["public"]["Enums"]["visibility_scope"];
const _visibility_scopeCheck: VisibilityScope extends SupabaseVisibilityScope ? SupabaseVisibilityScope extends VisibilityScope ? true : false : false = true;
void _visibility_scopeCheck;
type SupabaseWithdrawalAction = Database["public"]["Enums"]["withdrawal_action"];
const _withdrawal_actionCheck: WithdrawalAction extends SupabaseWithdrawalAction ? SupabaseWithdrawalAction extends WithdrawalAction ? true : false : false = true;
void _withdrawal_actionCheck;
type SupabaseWithdrawalStatus = Database["public"]["Enums"]["withdrawal_status"];
const _withdrawal_statusCheck: WithdrawalStatus extends SupabaseWithdrawalStatus ? SupabaseWithdrawalStatus extends WithdrawalStatus ? true : false : false = true;
void _withdrawal_statusCheck;
type SupabaseYieldDistributionStatus = Database["public"]["Enums"]["yield_distribution_status"];
const _yield_distribution_statusCheck: YieldDistributionStatus extends SupabaseYieldDistributionStatus ? SupabaseYieldDistributionStatus extends YieldDistributionStatus ? true : false : false = true;
