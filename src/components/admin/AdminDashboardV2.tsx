...
function AdminDashboardV2() {
   ...
   *
   * Then update the UI to show per-asset cards instead of aggregated values.
   */
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your yield fund platform</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-sm font-semibold text-amber-600">
              ⚠️ Needs Update: Per-Asset Display Required
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              Service layer must return aumByAsset array
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Service layer must return interest24hByAsset array
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Requiring admin action</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Fund management system operational status overview</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Latest investor registrations and activity</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {recentInvestors.map((investor) => (
              <div key={investor.id} className="flex items-center justify-between border-b pb-2">
                ...
                <div className="text-right">
                  <div className="text-sm text-amber-600 font-medium">Per-asset view needed</div>
                  <div className="text-sm text-muted-foreground">
                    {Object.keys(investor.portfolioDetails.assetBreakdown).length} position
                    {Object.keys(investor.portfolioDetails.assetBreakdown).length !== 1 ? "s" : ""}
                  </div>
                </div>
              </div>
            ))}
