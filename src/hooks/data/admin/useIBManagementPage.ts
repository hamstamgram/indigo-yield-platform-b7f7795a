...
export function useIBProfiles() {
  return useQuery<IBProfile[]>({
    ...
    queryFn: async (): Promise<IBProfile[]> => {
      ...
      const funds =
        allFundIds.length > 0 ? await fundService.getFundsByIds(allFundIds) : [];
      ...
      const ibProfiles: IBProfile[] = (profiles || []).map((p) => {
        ...
        txEarnings.forEach((t) => {
          const asset =
            t.asset || (t.fund_id ? fundToAsset.get(t.fund_id) : null) || "UNKNOWN";
          ...
          allocs.forEach((a) => {
            if (a.fund_id) {
              ...
              earningsByAsset[asset] =
                (earningsByAsset[asset] || 0) + Number(a.ib_fee_amount || 0);
            }
          });
          allocs.forEach((a) => {
