...
import { reportRecipientsService } from "@/services/admin/reportRecipientsService";
...
export function useReportRecipients(investorId: string) {
  return useQuery<string[]>({
    queryKey: ["admin", "reportRecipients", investorId],
...
export function useUpdateReportRecipients(investorId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin", "reportRecipients", investorId] });
    },
    onError: (error: Error) => {
      toast({
        title: "Update Failed",
