...
async function showImportSummary() {
  ...
  try {
    ...
    const sortedInvestors = Object.values(investorMap).sort((a, b) => a.name.localeCompare(b.name));
    ...
    const sortedAssets = Object.entries(assetTotals).sort((a, b) => {
    ...
    for (const [asset, data] of sortedAssets) {
      ...
      console.log(
        `      Est. USD Value: $${usdValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      );
    }

    console.log("\n----------------------------------------");
    console.log(
      `📊 TOTAL PLATFORM VALUE (Est.): $${totalUSDValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    );
    ...
    for (let i = 0; i < largestHoldings.length; i++) {
      ...
      console.log(
        `      Portfolio: ${investor.positions.map((p) => `${p.balance.toFixed(2)} ${p.asset}`).join(", ")}`
      );
      console.log(
        `      Est. Value: $${usdValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      );
    }
