...
test.describe("Portfolio Page - /dashboard/portfolio", () => {
  ...
  test("should display allocation breakdown", async ({ page }) => {
    ...
    const hasAllocation = await helpers.elementExists(
      "text=Allocation, text=Distribution, text=Breakdown"
    );
  ...
  test("should navigate to position details", async ({ page }) => {
    ...
    const positionLinks = await page.$$(
      'a[href*="position"], button:has-text("View"), button:has-text("Details")'
    );
