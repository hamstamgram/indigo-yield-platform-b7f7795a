...
test.describe("Performance Page - /dashboard/performance", () => {
  ...
  test("should test time period filter", async ({ page }) => {
    ...
    const periodButtons = await page.$$(
      'button:has-text("1M"), button:has-text("3M"), button:has-text("6M")'
    );
  ...
  test("should display portfolio growth over time", async ({ page }) => {
    ...
    expect(
      hasLineChart || hasAreaChart || (await helpers.elementExists(".recharts-wrapper"))
    ).toBeTruthy();
  ...
  test("should display performance by fund/asset", async ({ page }) => {
    ...
    const hasDistributionChart = await helpers.elementExists(".recharts-pie, .recharts-bar");
  ...
  test("should display export options", async ({ page }) => {
    ...
    const hasExportIcon = await helpers.elementExists(
