...
test.describe('Recurring Deposits Page - /transactions/recurring', () => {
  ...
  test('should have create new recurring deposit button', async ({ page }) => {
    const createButton = await page.$('button:has-text("New"), button:has-text("Create"), a[href*="new"]');
  ...
  test('should have pause/resume action', async ({ page }) => {
    const pauseButtons = await page.$$('button:has-text("Pause"), button:has-text("Resume"), [role="switch"]');
  ...
  test('should have cancel/delete action', async ({ page }) => {
    const deleteButtons = await page.$$('button:has-text("Cancel"), button:has-text("Delete"), button[aria-label*="Delete"]');
    ...
    if (deleteButtons.length > 0) {
      ...
      // Close confirmation
      const cancelButton = await page.$('button:has-text("Cancel"), button:has-text("No"), [aria-label*="Close"]');
      if (cancelButton) {
        await cancelButton.click();
        await page.waitForTimeout(300);
      }
  ...
  test('should display fund information', async ({ page }) => {
    const hasFundInfo = await helpers.elementExists(
      'text=Fund, text=Asset, text=Investment'
    );
  ...
  test('should show transaction history link', async ({ page }) => {
    const historyLinks = await page.$$('button:has-text("History"), a:has-text("History"), button:has-text("View Payments")');
  ...
  test('should display total recurring amount', async ({ page }) => {
    const hasTotal = await helpers.elementExists(
      'text=Total, text=Sum, text=Total Monthly'
    );
