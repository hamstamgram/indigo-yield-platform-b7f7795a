...
test.describe("Pending Transactions Page - /transactions/pending", () => {
  ...
  test("should have cancel action for pending transactions", async ({ page }) => {
    ...
    if (cancelButtons.length > 0) {
      ...
      const closeButton = await page.$(
        'button:has-text("No"), button:has-text("Close"), [aria-label*="Close"]'
      );
      if (closeButton) {
  ...
  test("should display refresh or reload option", async ({ page }) => {
    const refreshButton = await page.$(
      'button:has-text("Refresh"), button[aria-label*="Refresh"], button[aria-label*="Reload"]'
    );
  ...
  test("should show batch actions if available", async ({ page }) => {
    ...
    const selectAllCheckbox = await page.$(
      'input[type="checkbox"][aria-label*="Select all"], input[type="checkbox"][name="select-all"]'
    );
    ...
    if (selectAllCheckbox) {
      ...
      const batchActions = await page.$$(
        'button:has-text("Cancel Selected"), button:has-text("Export Selected")'
      );
