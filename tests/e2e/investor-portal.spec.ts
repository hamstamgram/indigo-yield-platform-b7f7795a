...
test.describe('Investor Portal - Dashboard View', () => {
  ...
  test('should display investor dashboard overview', async ({ page }) => {
    ...
    // Verify page header
    const pageTitle = page.locator('h1, h2').filter({ hasText: /overview/i }).first();
  ...
  test('should show holdings by token section', async ({ page }) => {
    ...
    // Look for token holdings display
    const holdingsSection = page.locator('text=/holdings|positions|balance/i').first();
    if (await holdingsSection.count() > 0) {
      await expect(holdingsSection).toBeVisible();
    }

    // Should display token symbols (USDT, USDC, etc.)
    const bodyText = await page.locator('body').textContent();
    if (bodyText && bodyText.length > 100) {
      const hasTokenFormat = bodyText.includes('USDT') || bodyText.includes('USDC') || bodyText.includes('USD');
  ...
  test('should show recent transactions preview', async ({ page }) => {
    ...
    const recentTxSection = page.locator('text=/recent.*transaction/i, text=/latest.*transaction/i');
    const hasRecentTx = await recentTxSection.first().isVisible().catch(() => false);
  ...
  test('should handle empty portfolio state gracefully', async ({ page }) => {
    ...
    const hasEmptyState = await page.locator('text=/no position/i, text=/get started/i').first().isVisible().catch(() => false);
    const hasError = await page.locator('[role="alert"]:has-text("error")').isVisible().catch(() => false);
  ...
  test('should display pending withdrawals notification', async ({ page }) => {
    ...
    // Look for pending withdrawal indicators (if any exist)
    const pendingIndicator = page.locator('text=/pending.*withdrawal/i, [data-testid="pending-withdrawals"]');

    // This is optional - user may or may not have pending withdrawals
  ...
  test('should be responsive on mobile viewport', async ({ page }) => {
    ...
    // Navigation should work on mobile (may be hamburger menu)
...
test.describe('Investor Portal - Position Viewing', () => {
  ...
  test('should display all investor positions', async ({ page }) => {
    ...
    const positionElements = page.locator('[data-testid*="position"], [data-testid*="fund"], table tbody tr');
  ...
  test('should display yield history', async ({ page }) => {
    ...
    // Look for yield data
    const yieldContent = page.locator('table, [data-testid*="yield"], text=/yield/i');
    if (await yieldContent.count() > 0) {
      await expect(yieldContent.first()).toBeVisible();
    }
  });
  ...
  test('should filter positions by asset type', async ({ page }) => {
    ...
    const filterControl = page.locator('[data-testid*="filter"], select, button:has-text("Filter")');

    if (await filterControl.first().isVisible().catch(() => false)) {
...
test.describe('Investor Portal - Transaction History', () => {
  ...
  test('should navigate to transactions page', async ({ page }) => {
    ...
    // Page should have heading
    const heading = page.locator('h1, h2').filter({ hasText: /transaction/i }).first();
    await expect(heading).toBeVisible();
  });
  ...
  test('should navigate to transaction detail page', async ({ page }) => {
    ...
    if (await transactionRow.isVisible().catch(() => false)) {
      ...
      const navigated = url.includes('/transactions/') || await modal.isVisible().catch(() => false);
  ...
  test('should filter transactions by date range', async ({ page }) => {
    ...
    const dateFilter = page.locator('[data-testid*="date"], input[type="date"], button:has-text("Date")');
...
test.describe('Investor Portal - Withdrawal Request Flow', () => {
  ...
  test('should access withdrawal request interface', async ({ page }) => {
    ...
    const withdrawalButton = page.locator('button:has-text("Withdraw"), a:has-text("Withdraw"), a[href*="withdrawal"]');

    if (await withdrawalButton.first().isVisible().catch(() => false)) {
      ...
      const hasWithdrawalUI =
        await withdrawalForm.isVisible().catch(() => false) ||
        url.includes('withdrawal');
  ...
  test('should display withdrawal request form', async ({ page }) => {
    // Try to access withdrawal settings or transactions page
    await page.goto('/investor/settings');
    await helpers.waitForPageLoad();

    // Look for withdrawal section
    const withdrawalSection = page.locator('text=/withdrawal/i, [data-testid*="withdrawal"]');

    // Withdrawal functionality should exist somewhere
    const count = await withdrawalSection.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });
  ...
  test('should validate withdrawal amount', async ({ page }) => {
    ...
    if (await withdrawalButton.isVisible().catch(() => false)) {
      ...
      const amountInput = page.locator('input[type="number"], input[name*="amount" i], input[placeholder*="amount" i]');
  ...
  test('should display pending withdrawal requests', async ({ page }) => {
    ...
    // Look for pending withdrawals section
    const pendingSection = page.locator('text=/pending.*withdrawal/i, [data-testid*="pending"]');

    // May or may not have pending withdrawals
  ...
  test('should allow cancellation of pending withdrawals', async ({ page }) => {
    ...
    const pendingRow = page.locator('tr:has-text("pending")').filter({ hasText: /withdrawal/i }).first();
...
test.describe('Investor Portal - Statement and Report Viewing', () => {
  ...
  test('should navigate to statements page', async ({ page }) => {
    ...
    // Page should have heading
    const heading = page.locator('h1, h2').filter({ hasText: /statement/i }).first();
    await expect(heading).toBeVisible();
  });
  ...
  test('should display statement period information', async ({ page }) => {
    ...
    if (rowCount > 0) {
      ...
      const hasPeriodInfo = rowText.match(/\d{4}|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|Q\d/);
  ...
  test('should display available documents', async ({ page }) => {
    ...
    const hasContent =
      await documentsList.first().isVisible().catch(() => false) ||
      await emptyState.isVisible().catch(() => false);
...
test.describe('Investor Portal - Navigation and Accessibility', () => {
  ...
  test('should logout successfully', async ({ page }) => {
    ...
    if (await logoutButton.first().isVisible().catch(() => false)) {
      ...
      const url = page.url();
      const onLoginPage = url.includes('login') || url.includes('auth') || url === new URL('/', page.url()).href;
...
test.describe('Investor Portal - Performance and Responsive Design', () => {
  ...
  test('should handle network errors gracefully', async ({ page }) => {
    ...
    const hasErrorBoundary = await page.locator('.error-boundary, text=/something went wrong/i').isVisible().catch(() => false);
