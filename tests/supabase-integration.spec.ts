...
test.describe("Supabase Database Integration Tests", () => {
  ...
  test.describe("Row-Level Security (RLS) Policies", () => {
    ...
    test("should prevent users from reading other profiles", async () => {
      ...
      const { data, error } = await userClient.from("profiles").select("*").limit(10);
    ...
    test("should prevent users from deleting their own profile", async () => {
      ...
      const { error } = await userClient.from("profiles").delete().eq("id", userId);
  ...
  test.describe("Admin Data Access", () => {
    test("should allow admins to read all profiles", async () => {
      try {
        ...
        // Fetch all profiles
        const { data, error } = await adminClient.from("profiles").select("*").limit(10);
...
test.describe("Supabase Storage Integration Tests", () => {
  ...
  test.describe("File Upload", () => {
    ...
    test("should upload image file", async () => {
      ...
      // Upload image
      const { data, error } = await userClient.storage.from("documents").upload(filePath, pngData, {
    ...
    test("should enforce file size limits", async () => {
      ...
      const { error } = await userClient.storage.from("documents").upload(filePath, largeContent);
  ...
  test.describe("File Download", () => {
    test("should download uploaded file", async () => {
      ...
      // Download file
      const { data, error } = await userClient.storage.from("documents").download(filePath);
  ...
  test.describe("Signed URLs", () => {
    ...
    test("should expire signed URLs after timeout", async () => {
      ...
      const { data } = await userClient.storage.from("documents").createSignedUrl(filePath, 1); // 1 second expiry
  ...
  test.describe("Storage RLS", () => {
    test("should prevent access to other users files", async () => {
      ...
      const { data, error } = await userClient.storage.from("documents").download(otherUserPath);
