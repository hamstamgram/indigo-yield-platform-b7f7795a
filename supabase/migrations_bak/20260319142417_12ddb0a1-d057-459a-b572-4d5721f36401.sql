-- Fix yield_allocations INSERT: correct column names to match actual table schema
-- Columns fixed: gross_yield->gross_amount, net_yield->net_amount, fee_percentage->fee_pct, 
--                ib_percentage->ib_pct, opening_balance->position_value_at_calc
-- Removed: created_by, purpose (don't exist in table)
-- Added: ownership_pct
...
BEGIN
  ...
  FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
    ...
    -- FIXED: Use correct column names matching yield_allocations table schema
    INSERT INTO yield_allocations (
      ...
      CASE WHEN v_opening_aum <> 0 THEN v_alloc.current_value / v_opening_aum ELSE 0 END,
      v_yield_tx_id
    );
