-- Fix: Remove double fee deduction in apply_segmented_yield_distribution_v5
-- The YIELD transaction already records the NET amount (gross - fee - ib).
-- The separate FEE transaction on the investor subtracts the fee a SECOND time.
-- This migration removes the redundant FEE debit on the investor while keeping
-- fee_allocations, platform_fee_ledger, and FEE_CREDIT to fees account intact.
...
BEGIN
  ...
  FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
    ...
    -- YIELD transaction: records NET amount (gross - fee) for the investor
    ...
    -- FEE handling: NO separate FEE transaction on the investor.
    -- The fee is already deducted in the net calculation (net = gross - fee).
    -- We only record fee_allocations (audit), platform_fee_ledger (tracking),
    -- and FEE_CREDIT to the fees account (platform income).
    IF v_alloc.fee > 0 AND v_alloc.account_type <> 'fees_account' THEN
      -- fee_allocations: audit trail (no debit_transaction_id since no FEE tx on investor)
      ...
      -- FEE_CREDIT to fees account: platform receives the fee income
      SELECT apply_investor_transaction(
        p_fund_id := p_fund_id,
      ...
      -- platform_fee_ledger: fee tracking record
    ...
    -- IB commission: credit to the introducing broker
