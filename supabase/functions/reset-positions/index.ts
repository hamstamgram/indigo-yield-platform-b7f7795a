...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "preview") {
      ...
      // Get total AUM
      const { data: aumData } = await supabase.from("investor_positions").select("current_value");
      const totalAUM = (aumData || []).reduce(
        (sum: number, p: { current_value: number | null }) => sum + Number(p.current_value || 0),
        0
      );
    ...
    if (action === "execute") {
      ...
      // Audit log: position reset
      await supabase.from("audit_log").insert({
        action: "POSITION_RESET",
        entity: "investor_positions",
        entity_id: "all",
        actor_user: user.id,
        meta: { reset_type: "execute", admin_email: profile.email, result: data },
      });
    ...
    if (action === "full-reset") {
      ...
      // Audit log: full data reset
      await supabase.from("audit_log").insert({
        action: "FULL_DATA_RESET",
        entity: "investor_positions",
        entity_id: "all",
        actor_user: user.id,
        meta: { reset_type: "full-reset", admin_email: profile.email, result: data },
      });
    ...
    return new Response(
      JSON.stringify({ error: "Invalid action. Use: preview, execute, full-reset, or history" }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      }
