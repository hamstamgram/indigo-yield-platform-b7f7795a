  ...
  try {
      ...
      try {
        ...
        if (sheet) {
          ...
          for (const row of rows) {
            const email = (row[mapping.investors.mappings.email] || '').toString().trim().toLowerCase();
            if (!email) continue;
    ...
    if (importType === 'transactions' || importType === 'full') {
      try {
        ...
        if (sheet) {
          ...
          for (const row of rows) {
            const email = (row[mapping.transactions.mappings.investor_email] || '').toString().trim().toLowerCase();
            ...
            const fundCode = asset === 'BTC' ? 'BTCYF' : 
                           asset === 'ETH' ? 'ETHYF' : 
                           asset.startsWith('USD') ? 'USDTYF' : null;
        ...
        for (const sheetName of mapping.daily_nav.sheets) {
          ...
          for (const row of rows) {
            ...
            const navData = {
              ...
              gross_return_pct: row[mapping.daily_nav.columns.gross_return_pct] == null ? 
                null : Number(row[mapping.daily_nav.columns.gross_return_pct]),
              net_return_pct: row[mapping.daily_nav.columns.net_return_pct] == null ? 
                null : Number(row[mapping.daily_nav.columns.net_return_pct]),
    ...
    await supabase
      ...
      .update({
        ...
        rows_processed: results.inserted.investors + results.inserted.transactions + results.inserted.daily_nav,
        rows_succeeded: results.inserted.investors + results.inserted.transactions + results.inserted.daily_nav,
