...
serve(async (req) => {
  ...
  try {
    ...
    for (const sheetName of ['BTC Yield Fund', 'ETH Yield Fund', 'USDT Yield Fund']) {
      ...
      const fundCode = sheetName.startsWith('BTC') ? 'BTCYF' :
                      sheetName.startsWith('ETH') ? 'ETHYF' : 'USDTYF';
      ...
      for (const row of rows) {
        ...
        if (dbAum !== undefined) {
          ...
          if (diff > 0.0001 && percentDiff > 0.01) {
            ...
            result.summary.maxAumVariance = Math.max(
              result.summary.maxAumVariance,
              percentDiff
            );
          }
        }
    ...
    if (investmentsSheet) {
      ...
      const investorMap = new Map(
        dbInvestors?.map((i: any) => [i.email, i]) || []
      );
    ...
    result.ok = result.summary.totalAumDiffs === 0 && 
