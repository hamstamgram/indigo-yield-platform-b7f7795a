...
async function runKYCChecks(supabase: any, investor: any): Promise<CheckResult[]> {
  ...
  const hasCompleteAddress = investor.address && investor.city && investor.state && investor.zip_code;
  checks.push({
...
async function runSanctionsChecks(supabase: any, investor: any): Promise<CheckResult[]> {
  ...
  checks.push({
    ...
    message: onUNSanctions
      ? 'Name matches UN sanctions list'
      : 'No UN sanctions list match',
  });
  ...
  checks.push({
    ...
    message: onEUSanctions
      ? 'Name matches EU sanctions list'
      : 'No EU sanctions list match',
  });
    ...
    if (transactions[i].transaction_type === 'deposit') {
      for (let j = i + 1; j < transactions.length; j++) {
        if (transactions[j].transaction_type === 'withdrawal') {
          const timeDiff = new Date(transactions[j].created_at).getTime() -
