-- Migration: Fix V5 inception month allocation + align preview with apply
-- Date: 2026-02-18
--
-- BUG 1 (INCEPTION): Both V5 functions skip allocation when opening_balance_sum = 0.
--   This breaks the fund's first month when all deposits happen within the period
--   (e.g., SOL fund Sept 2025: first deposit Sept 2, opening balance = 0).
--   FIX: When opening_balance_sum = 0, use post-flow balances as allocation basis.
--   Each investor's share = their balance after deposits / total balance after deposits.
--
-- BUG 2 (PREVIEW/APPLY MISMATCH): Preview used month-level algorithm, apply used
--   per-segment. This could produce different numbers for months with crystallization.
--   FIX: Rewrite preview to use identical per-segment algorithm as apply.
--
-- Affects ALL funds during their inception month.
...
DROP FUNCTION IF EXISTS public.preview_segmented_yield_distribution_v5(uuid, date, numeric, public.aum_purpose);

-- ============================================================
-- PREVIEW FUNCTION (per-segment, matching apply algorithm)
-- ============================================================
...
DECLARE
  ...
  -- Inception handling
  v_use_flow_basis boolean := false;
  v_alloc_base_sum numeric := 0;
...
BEGIN
  ...
  INSERT INTO _v5p_tot (investor_id) SELECT investor_id FROM _v5p_bal;
  ...
  -- ============================================================
  -- Process each segment (matches apply algorithm exactly)
  -- ============================================================
  FOR v_seg_idx IN 1..v_seg_count LOOP
    ...
    -- Skip negative/zero yield segments
    IF v_seg_yield <= 0 THEN
    ...
    -- Normal: opening_balance (pre-flow position)
    -- Inception (opening_balance_sum = 0): post-flow balance (deposits)
    v_use_flow_basis := (v_opening_balance_sum <= 0);
    IF v_use_flow_basis THEN
      SELECT COALESCE(SUM(balance), 0) INTO v_alloc_base_sum
      FROM _v5p_bal WHERE balance > 0;
    ELSE
      v_alloc_base_sum := v_opening_balance_sum;
    END IF;

    IF v_alloc_base_sum <= 0 THEN
      v_segments_out := v_segments_out || jsonb_build_object(
        'seg_idx', v_seg_idx, 'start', v_seg_start, 'end', v_seg_end,
        'closing_aum', v_seg_closing_aum, 'yield', v_seg_yield, 'skipped', true,
        'opening_balance_sum', v_opening_balance_sum, 'flows', v_seg_flows,
    ...
    -- Step 5: Allocate yield proportionally
    DECLARE
    ...
    BEGIN
      FOR v_inv IN
        ...
        FROM _v5p_bal b
        WHERE CASE WHEN v_use_flow_basis THEN b.balance > 0 ELSE b.opening_balance > 0 END
      LOOP
        v_share := CASE WHEN v_use_flow_basis
          THEN v_inv.balance / v_alloc_base_sum
          ELSE v_inv.opening_balance / v_opening_balance_sum END;
        ...
        -- Header totals (exclude fees_account)
        IF v_inv.investor_id != v_fees_account_id THEN
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY[
      ...
      'inception_month_flow_basis', 'ib_in_running_balance', 'fees_account_yield',
      'dust_to_fees_account', 'crystal_consolidation', 'aum_only_input',
      'inception_date_period_start'
    ]
  );
END;
$$;
...
-- ============================================================
-- APPLY FUNCTION (per-segment with inception fix)
-- ============================================================
...
BEGIN
  ...
  FOR v_seg_idx IN 1..v_seg_count LOOP
    ...
    -- Step 1: Snapshot opening_balance BEFORE flows
    ...
    v_segments_meta := v_segments_meta || jsonb_build_object(
      ...
      'opening_balance_sum', v_opening_balance_sum, 'flows', v_seg_flows,
      'inception', v_use_flow_basis,
      'investors', (SELECT count(*) FROM _v5_bal WHERE
        CASE WHEN v_use_flow_basis THEN balance > 0 ELSE opening_balance > 0 END)
    );
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY[
      ...
      'inception_month_flow_basis', 'crystal_consolidation', 'aum_only_input',
      'segment_notes_in_tx', 'inception_date_period_start', 'visibility_scope_control',
