-- Fix route_withdrawal_to_fees to use approved_amount/requested_amount instead of NULL processed_amount
...
BEGIN
  ...
  VALUES (v_request.investor_id, v_request.fund_id, 'FEE', -ABS(v_amount), v_fund_asset, v_request.fund_class, CURRENT_DATE, p_reason, 'fee_route_' || p_request_id::text, auth.uid(), false);

  -- Create FEE_CREDIT transaction (credit to fees account)
  INSERT INTO public.transactions_v2 (investor_id, fund_id, type, amount, asset, fund_class, tx_date, notes, reference_id, created_by, is_voided)
  VALUES (v_fees_account_id, v_request.fund_id, 'FEE_CREDIT', ABS(v_amount), v_fund_asset, v_request.fund_class, CURRENT_DATE, 'Fee from ' || LEFT(v_request.investor_id::text, 8), 'fee_credit_' || p_request_id::text, auth.uid(), false);
  ...
  -- Mark withdrawal as completed and set processed_amount
  UPDATE public.withdrawal_requests 
  SET status = 'completed', completed_at = NOW(), processed_amount = v_amount 
