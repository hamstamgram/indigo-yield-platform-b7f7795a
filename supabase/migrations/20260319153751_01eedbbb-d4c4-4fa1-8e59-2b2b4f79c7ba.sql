-- Fix: Subtract IB commission from investor net yield
-- v_net was calculated as (gross - fee) but should be (gross - fee - ib)
-- Without this, IB credits create value from nothing (14.20 XRP discrepancy)
...
BEGIN
  ...
  IF v_opening_aum <> 0 THEN
    FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
      ...
      ELSE
        ...
        -- FIX: Subtract BOTH fee AND ib from gross to get true net
  ...
  FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
    ...
    -- YIELD transaction: records NET amount (gross - fee - ib) for the investor
    ...
    -- The fee is already deducted in the net calculation (net = gross - fee - ib).
