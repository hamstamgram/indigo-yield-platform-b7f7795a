-- Migration: Fix date truncation in V6 yield math
-- Date: 2026-03-05
--
-- Change:
-- Previously apply_segmented_yield_distribution_v5 and preview_segmented_yield_distribution_v5
-- aggressively forced all p_period_end dates to date_trunc('month', p_period_end) + interval '1 month - 1 day'.
-- This broke intra-month manual yields (e.g. Sept 4th) by shifting them to the end of the month.
-- This migration updates the logic to respect the exact p_period_end date.

-- =============================================================
-- 1. Apply Yield (respect precise dates)
-- =============================================================
...
BEGIN
  ...
  -- USE PRECISE DATES.
  -- Only start of month if it's a 'reporting' yield, otherwise use the exact date
  ...
  -- TEMP TABLE: compute per-investor allocations
  ...
  -- Dust = total yield - investor gross - fees_account gross
...
-- =============================================================
-- 2. Preview Yield (respect precise dates)
-- =============================================================
