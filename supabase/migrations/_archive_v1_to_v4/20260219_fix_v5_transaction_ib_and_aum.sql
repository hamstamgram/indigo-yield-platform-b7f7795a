-- Fix: Two core bugs in apply_segmented_yield_distribution_v5
--
-- BUG 1: Transaction (checkpoint) purpose skips ib_commission_ledger writes.
-- The REPORTING branch correctly writes IB_CREDIT transactions + ib_commission_ledger.
-- The TRANSACTION (checkpoint) ELSE branch writes yield_allocations and
-- investor_yield_events but silently drops IB commissions entirely.
-- This causes ib_allocation_count_matches invariant check (check 8) to fail:
-- ib_alloc_count=0 vs expected_count=1 for investors with IB parents.
-- Fix: Add ib_commission_ledger INSERT in the ELSE branch (NULL transaction_id,
-- trigger sync_ib_allocations_from_commission_ledger auto-creates ib_allocations).
--
-- BUG 2: After applying a backdated reporting yield, today's fund_daily_aum
-- is stale if it exists. V5 writes fund_daily_aum for historical v_period_end
-- but today's record (written by a prior tx_sync) shows pre-yield positions.
-- This causes fund_aum_matches_positions check (check 2) to fail when the
-- latest AUM record is from today but doesn't reflect post-yield positions.
-- Fix: After calculating v_final_positions_sum, UPDATE today's fund_daily_aum
-- for reporting yields where v_period_end < CURRENT_DATE.
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- FIX (Bug 1): IB commission ledger for transaction checkpoints.
      -- No IB_CREDIT transaction (NULL transaction_id); trigger auto-creates ib_allocations.
      -- payout_status defaults to 'pending' for transaction-purpose via trigger logic.
      IF v_alloc.total_ib > 0 AND v_alloc.ib_parent_id IS NOT NULL THEN
        INSERT INTO ib_commission_ledger (
          fund_id, yield_distribution_id, source_investor_id, source_investor_name,
          ib_id, ib_name, gross_yield_amount, ib_percentage, ib_commission_amount,
  ...
  -- FIX (Bug 2): Sync today's transaction-purpose AUM after a backdated reporting yield.
  -- Reporting yields create real transactions (YIELD/FEE_CREDIT/IB_CREDIT) that update
  -- positions via trg_ledger_sync. If a today-dated fund_daily_aum record exists from a
  -- prior tx_sync, it will show pre-yield positions and fail fund_aum_matches_positions.
  IF p_purpose = 'reporting'::aum_purpose AND v_period_end < CURRENT_DATE THEN
    UPDATE fund_daily_aum
    SET total_aum = v_final_positions_sum,
        source = 'yield_reporting_today_sync',
        updated_at = now()
    WHERE fund_id = p_fund_id
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY[
      ...
      'transaction_ib_commission_ledger', 'reporting_aum_today_sync'
    ]
  );
END;
$$;
