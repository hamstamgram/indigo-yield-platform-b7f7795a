-- Fix check 8 (ib_allocation_count_matches): scope to purpose='reporting' only.
--
-- RATIONALE: Transaction-purpose (checkpoint) distributions intentionally do NOT
-- create ib_allocations. They only record ib_pct/ib_amount in yield_allocations
-- as internal accounting. Formal IB payment records (ib_allocations) are only
-- created for reporting-purpose distributions (which fire real IB_CREDIT transactions).
-- This matches check 3 which is already scoped to purpose='reporting'.
...
BEGIN
  ...
  -- Fixed: scope to purpose='reporting' only (same as check 3).
  -- Transaction checkpoints record ib_pct in yield_allocations for internal accounting,
  -- but do NOT create formal ib_allocations (no IB_CREDIT transaction is fired).
  -- IB payment records (ib_allocations) only exist for reporting-purpose distributions.
  WITH ib_count AS (
