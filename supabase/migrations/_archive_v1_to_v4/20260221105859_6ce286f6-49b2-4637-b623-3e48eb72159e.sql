...
-- Fix apply_investor_transaction: 
-- 1. Change p_tx_type from transaction_type to tx_type enum
-- 2. Fix audit_logs -> audit_log table name
-- 3. Fix audit column names to match actual schema

DROP FUNCTION IF EXISTS public.apply_investor_transaction(uuid, uuid, transaction_type, numeric, date, text, uuid, text, aum_purpose);
...
BEGIN
    ...
    -- 6. Log the administrative action (fixed: audit_log not audit_logs, correct column names)
    ...
    RETURN json_build_object(
        ...
        'message', p_tx_type || ' successfully applied without AUM crystallization'
    );
END;
$function$;
