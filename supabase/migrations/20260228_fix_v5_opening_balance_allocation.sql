-- Migration: Fix V5 engine to match Excel spreadsheet
-- Date: 2026-02-28
--
-- 5 changes from V5_VS_EXCEL_COMPREHENSIVE_AUDIT.md:
--   A) Allocation timing: yield to opening-balance holders only (Discrepancy #1)
--   B) Segment boundary: >= instead of > for day-1 flows (Discrepancy #3)
--   C) Remove is_active gate: use transactions_v2 to find investors (Discrepancies #4, #5)
--   D) Dust routing: route to fees_account, no fee/IB on dust (Discrepancy #7)
--   E) Fix IB comments: "IB from GROSS" not "from NET" (Discrepancy #8)
...
BEGIN
  ...
  -- ============================================================
  -- TEMP TABLES
  -- Change A: Added opening_balance column for pre-flow snapshot
  -- ============================================================
  ...
  -- Change C: Replace investor_positions join with transactions_v2 query
  -- This includes ALL investors with any transactions in the fund,
  -- regardless of is_active flag on investor_positions.
  -- ============================================================
  ...
  -- Change B: >= instead of > for day-1 flows
  -- ============================================================
  ...
  -- Change A: opening_balance allocation timing
  ...
  FOR v_seg_idx IN 1..v_seg_count LOOP
    ...
    UPDATE _v5_bal SET opening_balance = balance;
    ...
    -- opening_balance_sum = AUM before flows (what opening holders had)
    -- yield = closing_aum - opening_balance_sum - flows
    -- This isolates the actual yield from capital movements
    ...
    -- Investors with opening_balance = 0 (new depositors) get ZERO yield
