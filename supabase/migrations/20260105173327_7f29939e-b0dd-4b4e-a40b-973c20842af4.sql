-- Fix yield distribution v3 functions
-- Issues:
-- 1. Preview opening AUM fallback uses p_new_aum instead of positions sum
-- 2. Apply function uses non-existent full_name column
-- 3. Apply function uses wrong column names (tx_type, effective_date)
-- 4. Apply function missing required columns in transactions_v2 insert
-- 5. INDIGO FEES lookup uses full_name instead of account_type
...
BEGIN
  ...
  v_is_month_end := (p_target_date = (date_trunc('month', p_target_date) + interval '1 month - 1 day')::date);
  ...
  FOR v_inv IN
    ...
    LEFT JOIN investor_fee_schedule ifs ON ifs.investor_id = ip.investor_id
      ...
      AND p_target_date >= ifs.effective_date
      AND (ifs.end_date IS NULL OR p_target_date <= ifs.end_date)
  ...
  LOOP
    ...
    v_reference_id := 'yield_' || v_fund_code || '_' || p_target_date::text || '_' || v_inv.investor_id::text;
    ...
    v_ib_pct := v_inv.ib_commission_pct;
    v_ib_amount := 0;
...
BEGIN
  ...
  INSERT INTO fund_aum_events (
    ...
    p_fund_id, p_target_date, now(), 'yield_distribution', 'yield_' || v_fund_code || '_' || p_target_date::text,
  ...
  LOOP
    ...
    INSERT INTO yield_distributions (
      ...
      p_fund_id, v_inv.investor_id, p_target_date, v_period_id,
    ...
    INSERT INTO transactions_v2 (
      ...
      v_inv.investor_id, p_fund_id, 'yield', p_target_date, v_inv_net,
    ...
    INSERT INTO fee_allocations (
      ...
      p_target_date, p_target_date, p_purpose::aum_purpose, p_actor_id
    );
    ...
    IF v_ib_parent_id IS NOT NULL AND v_ib_pct > 0 THEN
      ...
      INSERT INTO ib_allocations (
        ...
        p_target_date, v_period_id, p_purpose::aum_purpose, 'yield_fee', p_actor_id
      );
      ...
      INSERT INTO transactions_v2 (
        ...
        v_ib_parent_id, p_fund_id, 'ib_commission', p_target_date, v_ib_amount,
  ...
  IF v_indigo_fees_credit > 0 AND v_indigo_fees_id IS NOT NULL THEN
    ...
    INSERT INTO transactions_v2 (
      ...
      v_indigo_fees_id, p_fund_id, 'fee_credit', p_target_date, v_indigo_fees_credit,
      v_fund_asset, v_fund_class, 'yield_distribution', 'admin_only', true,
      'Fee credit from yield distribution: ' || v_fund_code || ' ' || p_target_date::text, p_actor_id
    );
