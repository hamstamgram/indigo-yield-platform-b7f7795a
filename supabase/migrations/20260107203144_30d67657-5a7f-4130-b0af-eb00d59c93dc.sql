-- Fix yield fee calculation:
-- 1. fee_pct is stored as whole number (18 = 18%), must divide by 100
-- 2. IB amount should be calculated on gross yield, not on fee
...
BEGIN
  ...
  FOR v_inv IN
    SELECT 
      ...
      -- FIX: fee_pct stored as whole number (18 = 18%), divide by 100
      COALESCE(ifs.fee_pct / 100, 0.20) as fee_pct,
      p.ib_parent_id,
      -- ib_percentage stored as whole number (2 = 2%)
  ...
  LOOP
    ...
    v_distributions := v_distributions || jsonb_build_object(
      ...
      'feePercentage', v_fee_pct * 100,  -- Display as whole number (18%)
...
BEGIN
  ...
  LOOP
    ...
    -- Create fee allocation record (stores INDIGO fee only)
