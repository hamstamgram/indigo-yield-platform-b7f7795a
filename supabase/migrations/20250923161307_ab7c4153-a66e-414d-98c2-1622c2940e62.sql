-- Fixed migration using actual fund class values
...
-- Create investor positions using actual fund classes from existing funds
...
SELECT 
  i.id,
  f.id,
  f.fund_class,  -- Use actual fund class from funds table
...
LIMIT 50;  -- Create multiple positions per investor

-- Populate yield sources from legacy positions data
