-- Fix apply_segmented_yield_distribution_v5: correct apply_investor_transaction calls
-- The function signature is: (p_fund_id, p_investor_id, p_tx_type, p_amount, p_tx_date, p_reference_id, p_admin_id, p_notes, p_purpose, p_distribution_id)
-- It does NOT have p_source or p_visibility parameters.
-- Parameter order: p_fund_id comes FIRST, then p_investor_id.
...
BEGIN
  ...
  FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
    ...
    IF v_alloc.fee > 0 AND v_alloc.account_type <> 'fees_account' THEN
      ...
      -- FEE_CREDIT visibility is auto-set to admin_only by trg_enforce_internal_visibility
