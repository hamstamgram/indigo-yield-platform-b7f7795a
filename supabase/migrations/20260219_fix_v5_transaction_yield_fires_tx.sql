-- Fix: apply_segmented_yield_distribution_v5 TRANSACTION branch must fire real YIELD txs.
--
-- BUG: The TRANSACTION (checkpoint) branch was inserting yield_allocations with
-- transaction_id=NULL and directly inserting investor_yield_events. No actual YIELD
-- transaction was created, so investor_positions never updated and the fund card
-- showed stale AUM.
--
-- FIX: Mirror the REPORTING branch - fire apply_transaction_with_crystallization for
-- a YIELD tx, but keep visibility_scope='admin_only' (set by the sync trigger based on
-- purpose='transaction'). investor_yield_events is created by the sync trigger from
-- the YIELD tx INSERT, then enriched here (same pattern as REPORTING).
--
-- Also: expand the AUM today-sync to cover BOTH reporting and transaction backdated
-- yields (not just reporting), so the fund card stays accurate after any historical yield.
--
-- DATA FIX: Patch distribution 94bd3206 (SOL Sep 2025 transaction yield) which was
-- created before this fix and has transaction_id=NULL in yield_allocations.
...
DECLARE
  ...
  -- Transaction results (used for both reporting and transaction purpose)
  v_tx_result jsonb;
  v_yield_tx_id uuid;
...
BEGIN
  ...
  LOOP
    ...
    IF p_purpose = 'reporting'::aum_purpose THEN
      -- REPORTING: Create actual YIELD transactions (investor_visible)
      v_yield_tx_id := NULL;
    ...
    ELSE
      -- TRANSACTION: Fire admin-only YIELD tx (positions update, visible to admin only).
      -- visibility_scope stays 'admin_only' - sync trigger derives from purpose='transaction'.
      v_yield_tx_id := NULL;
      IF v_alloc.total_net != 0 THEN
        v_tx_result := apply_transaction_with_crystallization(
          p_fund_id          := p_fund_id,
          ...
          p_amount           := v_alloc.total_net,
          p_tx_date          := v_tx_date,
          p_reference_id     := 'yield_v5_tx_' || v_distribution_id::text || '_' || v_alloc.investor_id::text,
          p_notes            := 'V5 transaction yield for ' || to_char(v_period_start, 'Mon YYYY') ||
                                E'\n' || COALESCE(v_alloc.seg_detail::text, ''),
          p_admin_id         := v_admin,
          p_purpose          := p_purpose,
          p_distribution_id  := v_distribution_id
      ...
      -- investor_yield_events created by sync trigger from YIELD tx; enrich it here
      IF v_yield_tx_id IS NOT NULL THEN
        UPDATE investor_yield_events SET
          investor_balance = (
            SELECT COALESCE(current_value, 0) FROM investor_positions
      ...
      -- IB commission ledger for transaction checkpoints
      IF v_alloc.total_ib > 0 AND v_alloc.ib_parent_id IS NOT NULL THEN
        INSERT INTO ib_commission_ledger (
          fund_id, yield_distribution_id, source_investor_id, source_investor_name,
          ib_id, ib_name, gross_yield_amount, ib_percentage, ib_commission_amount,
  ...
  -- Record AUM for the yield period
  ...
  -- Sync today's transaction-purpose AUM after any backdated yield (reporting or transaction).
  -- Uses INSERT ... ON CONFLICT to handle: no today record, or today record is voided
  -- (partial index only covers is_voided=false, so voided records don't conflict).
  IF v_period_end < CURRENT_DATE THEN
    INSERT INTO fund_daily_aum (fund_id, aum_date, total_aum, purpose, source, created_by, is_month_end)
    VALUES (p_fund_id, CURRENT_DATE, v_final_positions_sum, 'transaction', 'yield_today_sync', v_admin, false)
    ON CONFLICT (fund_id, aum_date, purpose) WHERE (is_voided = false)
    DO UPDATE SET total_aum = EXCLUDED.total_aum, source = 'yield_today_sync', updated_at = now();
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY[
      ...
      'dust_to_fees_account', 'transaction_yield_fires_admin_tx',
      'transaction_ib_commission_ledger', 'aum_today_sync_all_purposes'
    ]
  );
END;
$$;
...
-- ============================================================
-- DATA FIX: Patch distribution 94bd3206 (SOL Sep 2025 tx yield)
-- Created before this fix: yield_allocations.transaction_id=NULL,
-- investor_positions not updated, today's AUM stale/voided.
-- ============================================================
DO $$
DECLARE
  v_dist_id uuid := '94bd3206-9ae0-41cf-8d6b-fb98ddc94c91';
  v_fund_id uuid := '7574bc81-aab3-4175-9e7f-803aa6f9eb8f';
  v_admin_id uuid := '26ebf5ff-9755-43a4-984c-ac7e093a6416';
  v_lp_id uuid := 'd91f3eb7-bd47-4c42-ab4f-c4f20fb41b13';
  v_tx_result jsonb;
  v_yield_tx_id uuid;
  v_final_aum numeric;
BEGIN
  ...
  -- Create the missing YIELD transaction for INDIGO LP (+2 SOL, 2025-09-30, admin_only)
  v_tx_result := apply_transaction_with_crystallization(
    p_fund_id         := v_fund_id,
    p_investor_id     := v_lp_id,
    p_tx_type         := 'YIELD',
    p_amount          := 2.0,
    p_tx_date         := '2025-09-30'::date,
    p_reference_id    := 'yield_v5_tx_' || v_dist_id::text || '_' || v_lp_id::text,
    p_notes           := 'V5 transaction yield for Sep 2025 [retroactive fix]',
  ...
  IF v_yield_tx_id IS NULL THEN
    RAISE EXCEPTION 'Failed to create retroactive YIELD tx: %', v_tx_result;
  END IF;

  -- Link yield_allocations to the new transaction
  UPDATE yield_allocations
  SET transaction_id = v_yield_tx_id
  WHERE distribution_id = v_dist_id
    AND investor_id = v_lp_id
    AND transaction_id IS NULL;

  -- Enrich the investor_yield_events row (created by sync trigger from YIELD tx)
  UPDATE investor_yield_events SET
    gross_yield_amount = 2.0,
    net_yield_amount   = 2.0,
    fee_pct            = 0,
    fee_amount         = 0,
    ib_amount          = 0,
    period_start       = '2025-09-01'::date,
    period_end         = '2025-09-30'::date,
    days_in_period     = 30
  WHERE trigger_transaction_id = v_yield_tx_id AND is_voided = false;

  -- Sync today's AUM (positions are now 1252 after yield tx)
  SELECT COALESCE(SUM(current_value), 0) INTO v_final_aum
  FROM investor_positions
  WHERE fund_id = v_fund_id AND is_active = true;

  INSERT INTO fund_daily_aum (fund_id, aum_date, total_aum, purpose, source, created_by, is_month_end)
  VALUES (v_fund_id, CURRENT_DATE, v_final_aum, 'transaction', 'yield_today_sync', v_admin_id, false)
  ON CONFLICT (fund_id, aum_date, purpose) WHERE (is_voided = false)
  DO UPDATE SET total_aum = EXCLUDED.total_aum, source = 'yield_today_sync', updated_at = now();

  RAISE NOTICE 'Retroactive fix complete: yield_tx_id=%, position=% SOL', v_yield_tx_id, v_final_aum;
END $$;
