...
-- Comprehensive fix for force_delete_investor function
-- Includes all related tables and proper deletion order
...
BEGIN
  ...
  -- Create WITHDRAWAL transactions for each position as audit trail BEFORE deleting
  FOR v_position IN 
  ...
  -- Statement and report delivery tables (children first)
  DELETE FROM statement_email_delivery WHERE investor_id = p_investor_id OR user_id = p_investor_id;
  DELETE FROM statement_metadata WHERE investor_id = p_investor_id;
  ...
  -- User settings and sessions (using user_id)
  ...
  DELETE FROM secure_shares WHERE created_by = p_investor_id;
  DELETE FROM support_tickets WHERE user_id = p_investor_id;
  
  -- Finally delete profile (parent table)
