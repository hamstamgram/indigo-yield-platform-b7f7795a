-- Fix void_fund_daily_aum to also void yield_allocations
-- Part 1: Fix existing orphaned yield_allocations records
...
-- Part 2: Update void_fund_daily_aum to include yield_allocations voiding
...
BEGIN
  ...
  -- 1. Void yield_distributions (with is_voided = true)
  PERFORM set_config('indigo.canonical_rpc', 'true', true);
  PERFORM set_config('app.canonical_rpc', 'true', true);
  ...
  -- 2. Void yield_allocations (NEW - links to distributions by distribution_id)
  PERFORM set_config('indigo.canonical_rpc', 'true', true);
  PERFORM set_config('app.canonical_rpc', 'true', true);
  UPDATE yield_allocations
  SET is_voided = true
