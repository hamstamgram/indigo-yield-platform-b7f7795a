-- Migration: Cleanup Legacy Crystallization RPCs
-- Date: 2026-03-04
-- Context: Removes obsolete monolithic transaction RPCs and preflow functions.
-- Auto-crystallization is dead. Capital flows are handled independently now via `apply_investor_transaction`.

BEGIN;

-- Drop the monolithic legacy function
DROP FUNCTION IF EXISTS public.apply_transaction_with_crystallization(uuid, uuid, text, numeric, date, text, uuid, text, aum_purpose, numeric);
DROP FUNCTION IF EXISTS public.apply_transaction_with_crystallization(uuid, uuid, text, numeric, date, text, uuid, text, aum_purpose);
DROP PROCEDURE IF EXISTS public.apply_transaction_with_crystallization; -- just in case it was a proc

-- Drop preflow AUM logic, which relied on the dropped fund_aum_events table
DROP FUNCTION IF EXISTS public.ensure_preflow_aum(uuid, date, public.aum_purpose);
DROP FUNCTION IF EXISTS public.get_existing_preflow_aum(uuid, date, public.aum_purpose);

-- Remove the p_new_total_aum parameter from void_and_reissue_transaction
DROP FUNCTION IF EXISTS public.void_and_reissue_transaction(uuid, jsonb, uuid, text, numeric);
