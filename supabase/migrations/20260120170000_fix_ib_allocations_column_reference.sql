-- Fix: Correct column references in yield distribution functions
-- Issue: Functions reference iba.commission_rate but column is iba.ib_percentage
-- Issue: Functions reference iba.investor_id but column is iba.source_investor_id
...
BEGIN
  ...
  -- INDIGO FEES (fees_account) gets 0% fee
  FOR v_investor IN
    SELECT
      ip.investor_id,
...
'Apply yield using ADB method. FIXED: Uses ib_percentage and source_investor_id columns.';

COMMENT ON FUNCTION preview_adb_yield_distribution_v3 IS
'Preview ADB yield distribution. FIXED: Uses ib_percentage and source_investor_id columns.';
