-- Migration: Make crystallize_yield_before_flow idempotent for retry scenarios
-- Problem: When a deposit/withdrawal fails AFTER crystallization creates yield events but the AUM event gets voided,
--          the orphaned yield events block retries because of the unique constraint on reference_id.
-- Solution: Before creating new yield events, void any existing non-voided events for this fund/date/trigger.
...
begin
  -- P0-OPS-001a: MANDATORY purpose check
  ...
  -- =========================================================================
  -- CORE FIX: Validate p_closing_aum against positions as-of event date
  -- =========================================================================
  ...
  -- FIX: Remove ::text cast - pass p_purpose directly as aum_purpose
  ...
  if v_existing_preflow.aum_event_id is not null then
    ...
    return jsonb_build_object(
      ...
      'message', 'Reused existing preflow AUM',
      'validation', v_validation_result
    );
  end if;
  ...
  if v_yield_amount < 0 then
    raise exception 'Gross yield amount cannot be negative (closing AUM % is less than opening AUM %)',
      p_closing_aum, v_opening_aum;
  end if;
  ...
  -- Only process yield distribution if there IS positive yield
  if v_opening_aum > 0 and v_yield_amount > 0 then

    -- =========================================================================
    -- IDEMPOTENCY FIX: Void any orphaned yield artifacts from previous failed attempts
    -- =========================================================================

    -- 1. Void orphaned investor yield events for this fund/date
    ...
    -- 2. Delete orphaned fund_yield_snapshots (table has no void columns)
    delete from fund_yield_snapshots
    where fund_id = p_fund_id
      and snapshot_date = v_event_date
      and trigger_type = p_trigger_type;

    get diagnostics v_orphaned_snapshots = row_count;

    -- 3. Void orphaned yield_distributions for this fund/date/purpose
    ...
    where fund_id = p_fund_id
      and effective_date = v_event_date
      and purpose = p_purpose
      and distribution_type = p_trigger_type
      and is_voided = false;
    perform set_canonical_rpc(false);

    get diagnostics v_orphaned_distributions = row_count;

    -- Log if we voided orphaned records (helps with debugging)
    ...
    -- Find dust receiver (largest investor)
    ...
    -- Process each investor with ADB-weighted allocation
    for v_investor in
      select
        ip.investor_id,
        ip.current_value,
    ...
    loop
        ...
        select d::date as balance_date,
               coalesce(
                 (select end_of_day_balance
                  from investor_daily_balance idb
                  where idb.investor_id = v_investor.investor_id
                    and idb.fund_id = p_fund_id
                    and idb.balance_date = d::date),
                 (select coalesce(sum(t.amount), 0)
                  from transactions_v2 t
                  where t.investor_id = v_investor.investor_id
      ...
      if v_investor_yield > 0 then
        ...
        -- FIX: Divide by 100 since fee_pct is stored as percentage (15 for 15%)
  ...
  return jsonb_build_object(
    ...
    'zero_yield_skipped', v_yield_amount = 0,
    'validation', v_validation_result,
