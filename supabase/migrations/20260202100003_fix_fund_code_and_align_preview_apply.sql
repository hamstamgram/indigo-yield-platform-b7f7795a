-- Fix 1: apply_adb_yield_distribution_v3 references v_fund.fund_code but column is v_fund.code
-- Fix 2: preview_adb_yield_distribution_v3 ADB scope includes fees_account; apply excludes it
-- Fix 3: preview doesn't exempt IB accounts from fees; apply does (ib_fee_exempt)

-- ============================================================
-- FIX apply_adb_yield_distribution_v3: v_fund.fund_code → v_fund.code
-- ============================================================
...
BEGIN
  ...
  -- Reporting mode: derive period from p_period_end, optionally derive yield from transactions
  IF p_purpose = 'reporting'::aum_purpose THEN
    ...
    ELSE
      -- Fallback: use latest transaction month
    ...
    END IF;
    -- else: user provided a positive amount, use it as-is
  END IF;
  ...
  INSERT INTO audit_log (
    ...
    jsonb_build_object(
      'fund_id', p_fund_id,
      'fund_code', v_fund.code,  -- FIX: was v_fund.fund_code (column doesn't exist)
...
--   1) Exclude fees_account from ADB (match apply)
--   2) IB accounts are fee-exempt (match apply)
-- ============================================================
...
BEGIN
  ...
  -- Calculate total ADB (investor + IB positions only, exclude fees_account — matches apply)
  ...
  -- Calculate allocations (investor + IB only, exclude fees_account — matches apply)
  FOR v_investor IN
    SELECT
      ip.investor_id,
      ...
      -- IB accounts are fee-exempt (matches apply)
  ...
  LOOP
    ...
    -- IB commission on GROSS (matches apply)
    ...
    -- Net = gross - fee - ib (matches apply)
    v_net_share := v_gross_share - v_fee_share - v_ib_share;
