-- Migration: Remove AUM deviation blocker from all yield/AUM operations
--
-- Problem: Both validate_aum_against_positions and validate_aum_against_positions_at_date
-- block operations when entered AUM deviates >10% from position sum. This prevents
-- legitimate yield operations (e.g. first yield for small positions).
--
-- Fix: Always return valid=true. Keep deviation info as advisory, not a gate.
-- Applies to BOTH validator functions.
