-- Migration: Fix engine for event-driven yield distribution
-- Date: 2026-02-19
--
-- ROOT CAUSE: The yield engine has 4 core issues preventing it from matching
-- the Excel source of truth:
--
-- 1. crystallize_yield_before_flow creates CHECKPOINTS ONLY (no YIELD transactions)
--    → Positions don't change when yield is distributed before deposits/withdrawals
--
-- 2. crystallize uses ADB weighting instead of flat allocation
--    → For short inter-event periods, flat (current_value/total) matches Excel
--
-- 3. crystallize has NO IB commission handling
--    → IB parents don't earn commissions on crystallized yield
--
-- 4. V5 computes yield from opening balances + month flows instead of current positions
--    → When crystallize creates real transactions, V5 double-counts yield
--
-- FIX:
-- Part 1: apply_transaction_with_crystallization passes PRE-FLOW AUM to crystallize
-- Part 2: crystallize creates real YIELD/FEE_CREDIT/IB_CREDIT transactions
-- Part 3: V5 uses remaining_yield = recorded_aum - SUM(current_positions)
-- Part 4: V5 preview matches apply changes
--
-- This fix is BACKWARD COMPATIBLE:
-- Without crystallize creating real txs: SUM(positions) = opening + flows,
-- so V5's new formula = old formula. With real txs: V5 correctly computes remainder.

-- ============================================================
-- PART 1: apply_transaction_with_crystallization
--         Pass PRE-FLOW AUM to crystallize
-- ============================================================

DROP FUNCTION IF EXISTS public.apply_transaction_with_crystallization(uuid, uuid, text, numeric, date, text, numeric, uuid, text, aum_purpose, uuid);
...
BEGIN
  ...
  -- FIX: Compute PRE-FLOW AUM for crystallize (not post-flow).
  -- When admin provides p_new_total_aum, it includes the pending
  -- deposit/withdrawal. We subtract/add the flow amount to get
  -- the AUM as it was BEFORE this transaction (= the yield-bearing AUM).
  -- ====================================================================
  IF p_tx_type IN ('DEPOSIT', 'WITHDRAWAL', 'ADJUSTMENT') THEN
    ...
    IF v_fund_last_crystal_date IS NOT NULL AND v_fund_last_crystal_date < p_tx_date THEN
      IF p_new_total_aum IS NOT NULL THEN
        -- Admin provided POST-flow AUM; derive PRE-FLOW AUM
        IF p_tx_type = 'DEPOSIT' THEN
          v_crystal_closing_aum := p_new_total_aum - ABS(p_amount);
        ELSIF p_tx_type = 'WITHDRAWAL' THEN
          v_crystal_closing_aum := p_new_total_aum + ABS(p_amount);
        ELSE
          v_crystal_closing_aum := p_new_total_aum;
        END IF;
      ELSE
        -- No AUM provided; look up last recorded (already pre-flow)
        SELECT total_aum INTO v_crystal_closing_aum
        FROM fund_daily_aum
        WHERE fund_id = p_fund_id AND aum_date <= p_tx_date
          AND purpose = 'transaction' AND is_voided = false
        ORDER BY aum_date DESC LIMIT 1;

        IF v_crystal_closing_aum IS NULL THEN
          SELECT total_aum INTO v_crystal_closing_aum
          FROM fund_daily_aum
      ...
      IF v_crystal_closing_aum IS NOT NULL AND v_crystal_closing_aum > 0 THEN
        v_crystal_result := crystallize_yield_before_flow(
          p_fund_id := p_fund_id,
          p_closing_aum := v_crystal_closing_aum,
  ...
  INSERT INTO transactions_v2 (
    ...
    CASE
      ...
      WHEN p_tx_type = 'YIELD' THEN p_amount
      ELSE ABS(p_amount)
    END,
...
EXCEPTION
  WHEN unique_violation THEN
    ...
    IF v_existing_tx IS NOT NULL THEN
      RETURN jsonb_build_object(
        'success', true, 'idempotent', true, 'tx_id', v_existing_tx,
        'message', 'Transaction already exists (caught via unique_violation)'
      );
    END IF;
    RAISE;
END;
...
-- ============================================================
-- PART 2: crystallize_yield_before_flow
--         Create real YIELD/FEE_CREDIT/IB_CREDIT transactions
--         Use flat allocation (current_value / total)
--         Handle IB commissions and negative yield
-- ============================================================
...
-- ============================================================
-- PART 3: apply_segmented_yield_distribution_v5
--         Use remaining_yield = recorded_aum - SUM(current_positions)
--         Remove opening balance computation and month_flows
-- ============================================================
...
DECLARE
  ...
  -- Current positions (replaces opening balance computation)
  v_current_positions_sum numeric := 0;
  v_remaining_yield numeric := 0;
  v_is_negative_yield boolean := false;
...
BEGIN
  ...
  -- ============================================================
  -- TEMP TABLES (now populated from CURRENT POSITIONS, not ledger)
  -- ============================================================
  ...
  -- ============================================================
  -- FIX: Use CURRENT POSITIONS instead of ledger opening balances
  -- This gives correct results whether or not crystallization
  -- created real transactions during the period.
  -- ============================================================
  ...
  SELECT
    ...
    p.account_type::text,
    p.ib_parent_id,
    get_investor_ib_pct(ip.investor_id, p_fund_id, v_period_end),
    trim(COALESCE(p.first_name, '') || ' ' || COALESCE(p.last_name, ''))
  FROM investor_positions ip
  JOIN profiles p ON p.id = ip.investor_id
  WHERE ip.fund_id = p_fund_id AND ip.is_active = true AND ip.current_value > 0;

  -- Ensure fees_account is always present
  INSERT INTO _v5_bal (investor_id, balance, opening_balance, account_type, ib_parent_id, ib_rate, investor_name)
  SELECT p.id, COALESCE(ip.current_value, 0), COALESCE(ip.current_value, 0),
         'fees_account', NULL, 0,
         trim(COALESCE(p.first_name, '') || ' ' || COALESCE(p.last_name, ''))
  FROM profiles p
  LEFT JOIN investor_positions ip ON ip.investor_id = p.id AND ip.fund_id = p_fund_id
  WHERE p.id = v_fees_account_id
  ...
  -- ============================================================
  -- FIX: REMAINING YIELD = recorded_aum - SUM(current_positions)
  -- This is the yield that hasn't been distributed yet.
  -- If crystallize created real transactions, positions already
  -- reflect that yield, so remaining_yield is just the gap.
  -- ============================================================
  SELECT COALESCE(SUM(balance), 0) INTO v_current_positions_sum FROM _v5_bal;
  v_alloc_sum := v_current_positions_sum;
  v_remaining_yield := p_recorded_aum - v_current_positions_sum;
  v_is_negative_yield := (v_remaining_yield < 0);

  -- Build crystal marker metadata (for display only)
  FOR v_inv IN
  ...
  INSERT INTO yield_distributions (
    ...
    p_recorded_aum, v_current_positions_sum, 0, 0,
    0, 0, 0, 0, 0, 0, 0,
  ...
  -- ============================================================
  -- YIELD ALLOCATION (based on current positions, remaining yield)
  -- ============================================================
  IF v_remaining_yield != 0 AND v_alloc_sum > 0 THEN
    ...
    LOOP
      ...
      v_gross := ROUND((v_remaining_yield * v_share)::numeric, 8);
  ...
  -- Dust routing
  v_residual := v_remaining_yield - v_total_gross - v_fees_account_gross;
      ...
      IF v_alloc.total_net != 0 THEN
        ...
        IF v_yield_tx_id IS NOT NULL THEN
          ...
          UPDATE investor_yield_events SET
            ...
            END,
            fund_yield_pct = CASE
              WHEN v_current_positions_sum > 0
              THEN ROUND((v_remaining_yield / v_current_positions_sum * 100)::numeric, 6)
              ELSE 0
            END,
    ...
    ELSE
      ...
      IF v_alloc.total_net != 0 THEN
        INSERT INTO investor_yield_events (
          ...
          v_current_positions_sum, p_recorded_aum,
          v_alloc.total_gross,
          CASE WHEN v_alloc.investor_id = v_fees_account_id THEN 0
               ELSE get_investor_fee_pct(v_alloc.investor_id, p_fund_id, v_period_end) END,
          ...
          CASE WHEN v_total_gross > 0
               THEN ROUND((v_alloc.total_gross / v_total_gross * 100)::numeric, 6)
               ELSE 0 END,
          CASE WHEN v_current_positions_sum > 0
               THEN ROUND((v_remaining_yield / v_current_positions_sum * 100)::numeric, 6)
               ELSE 0 END,
  ...
  -- Transaction purpose: update crystallization date
  ...
  UPDATE yield_distributions SET
    ...
    summary_json = jsonb_build_object(
      'version', 'v5_current_positions',
      'current_positions_sum', v_current_positions_sum,
      'remaining_yield', v_remaining_yield,
      'is_negative_yield', v_is_negative_yield,
  ...
  VALUES (
    ...
    jsonb_build_object('current_positions_sum', v_current_positions_sum),
    jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'purpose', p_purpose::text, 'calculation_method', 'v5_current_positions',
      'remaining_yield', v_remaining_yield,
      'is_negative_yield', v_is_negative_yield,
      'crystals_consolidated', v_crystals_consolidated,
  ...
  RETURN jsonb_build_object(
    ...
    'days_in_period', (v_period_end - v_period_start + 1),
    'features', ARRAY[
      'current_positions_allocation', 'remaining_yield_formula',
      'negative_yield_support', 'per_investor_fees', 'ib_from_gross',
      'fees_account_yield', 'crystal_consolidation', 'aum_only_input',
      'inception_date_period_start', 'visibility_scope_control',
      'yield_event_enrichment', 'distribution_date_override',
      'dust_to_fees_account', 'transaction_checkpoint_only'
    ]
  );
END;
$$;
...
-- ============================================================
-- PART 4: preview_segmented_yield_distribution_v5
--         Match apply changes (current positions, remaining yield)
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- TEMP TABLES (from CURRENT POSITIONS)
  -- ============================================================
  ...
  -- FIX: REMAINING YIELD = recorded_aum - SUM(current_positions)
  SELECT COALESCE(SUM(balance), 0) INTO v_current_positions_sum FROM _v5p_bal;
  v_alloc_sum := v_current_positions_sum;
  v_remaining_yield := p_recorded_aum - v_current_positions_sum;
  v_is_negative_yield := (v_remaining_yield < 0);
  ...
  RETURN jsonb_build_object(
    ...
    'conservation_check', (v_total_gross = v_total_net + v_total_fees + v_total_ib),
    'calculation_method', 'v5_current_positions',
    'features', ARRAY[
      'current_positions_allocation', 'remaining_yield_formula',
      'negative_yield_support', 'per_investor_fees', 'ib_from_gross',
      'fees_account_yield', 'crystal_consolidation', 'aum_only_input',
      'inception_date_period_start', 'dust_to_fees_account',
      'fees_account_in_preview'
    ]
  );
END;
$$;
