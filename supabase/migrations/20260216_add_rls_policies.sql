...
CREATE POLICY admin_invites_select 
ON public.admin_invites FOR SELECT TO authenticated USING (is_admin());

CREATE POLICY onboarding_submissions_select 
ON public.onboarding_submissions FOR SELECT TO authenticated USING (is_admin());
