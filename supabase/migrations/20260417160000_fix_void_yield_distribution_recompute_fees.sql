-- ============================================================
-- P0 FIX: void_yield_distribution must recompute fees_account position
-- Date: 2026-04-17
-- ============================================================
-- void_yield_distribution currently only recomputes positions for
-- investors found via SELECT DISTINCT investor_id FROM transactions_v2
-- WHERE distribution_id = p_distribution_id. This misses the
-- fees_account if it received fee credits from OTHER distributions.
--
-- Also adds recompute_investor_position as safety net for EACH
-- voided transaction (dual path with trg_ledger_sync trigger).
--
-- Additionally sets status='voided' on voided distributions
-- (fix from 20260417120000 preserved).
-- ============================================================
...
BEGIN
  ...
  -- Recompute positions for ALL investors affected by this distribution
  ...
  -- ALWAYS recompute fees_account position for this fund
  -- (fees_account may have FEE_CREDIT from this distribution that
  -- wasn't caught by the distribution_id lookup above)
...
ALTER FUNCTION public.void_yield_distribution(uuid, uuid, text, boolean) OWNER TO "postgres";

COMMENT ON FUNCTION public.void_yield_distribution(uuid, uuid, text, boolean) IS 'Void a yield distribution with full cascade. v2.0 (2026-04-17): Added recompute_investor_position for each voided tx (dual path), always recomputes fees_account position, sets status=voided.';