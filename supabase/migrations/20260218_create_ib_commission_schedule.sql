-- Create IB Commission Schedule (mirrors investor_fee_schedule pattern)
--
-- Enables date-based, per-fund IB commission rate overrides.
-- Hierarchy: fees_account=0 -> ib_commission_schedule (fund-specific)
--   -> ib_commission_schedule (global) -> profiles.ib_percentage -> 0

-- ============================================================
-- 1. New table: ib_commission_schedule
-- ============================================================
...
COMMENT ON COLUMN public.ib_commission_schedule.ib_percentage IS
  'IB commission percentage stored as 0-100 (e.g., 5 = 5%). Input and display directly as X% without conversion.';

-- ============================================================
-- 2. RLS policies (mirror investor_fee_schedule)
-- ============================================================
...
-- ============================================================
-- 3. Auto-close trigger (mirror auto_close_previous_fee_schedule)
-- ============================================================
...
-- ============================================================
-- 4. Resolution function: _resolve_investor_ib_pct
-- ============================================================
...
COMMENT ON FUNCTION public._resolve_investor_ib_pct(uuid, uuid, date) IS
  'Resolves investor IB commission percentage. Checks: 1) fees_account=0, 2) ib_commission_schedule (fund-specific then global), 3) profiles.ib_percentage, 4) default 0%.';

-- ============================================================
-- 5. Update get_investor_ib_pct to use schedule
-- ============================================================
-- Drop old 2-param version
DROP FUNCTION IF EXISTS public.get_investor_ib_pct(uuid, uuid);

-- Create new 3-param version with date (DEFAULT CURRENT_DATE for backward compat)
...
COMMENT ON FUNCTION public.get_investor_ib_pct(uuid, uuid, date) IS
  'Centralized IB rate resolution: delegates to _resolve_investor_ib_pct which checks ib_commission_schedule (fund-specific then global), then profile default. fees_account always returns 0.';
