...
BEGIN
  IF NEW.type = 'YIELD'::public.tx_type AND NOT NEW.is_voided THEN
    ...
    IF v_alloc IS NOT NULL THEN
      v_investor_share_pct := COALESCE(v_alloc.investor_share_pct, 0);
...
SET 
  investor_share_pct = COALESCE(ya.investor_share_pct, iye.investor_share_pct),
  ...
  investor_balance = CASE
    WHEN COALESCE(ya.investor_share_pct, 0) > 0 AND COALESCE(yd.recorded_aum, 0) > 0
    THEN ya.investor_share_pct * yd.recorded_aum
