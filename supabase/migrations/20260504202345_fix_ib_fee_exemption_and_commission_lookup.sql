-- =============================================================
-- Fix #1: IB commission lookup on IB parent (not investor)
-- Fix #2: IB accounts exempted from management fees
-- Fix #3: Alex Jacobs IB commission schedule for SOL fund
-- 2026-05-04 | Diagnosis confirmed both bugs cause wrong fee/IB
-- calculations for yield distributions involving IB accounts
-- =============================================================

-- =============================================================
-- Fix #2: _resolve_investor_fee_pct — exempt IB accounts
-- Current: only exempts fees_account
-- Fix: exempt both fees_account AND ib account types
-- =============================================================
...
BEGIN
  ...
  IF v_account_type IN ('fees_account', 'ib') THEN
    RETURN 0;
  END IF;
...
-- =============================================================
-- Fix #1 + #2: apply_segmented_yield_distribution_v5
-- (1) IB rate lookup now uses ib_parent_id instead of investor_id
-- (2) IB account short-circuit exemption added
-- =============================================================
...
BEGIN
  ...
  IF v_pre_day_aum > 0 THEN
    FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
      ...
      -- FIX: Exempt both fees_account AND ib from fees/IB charges
      IF v_alloc.account_type IN ('fees_account', 'ib') THEN
        v_fee_pct := 0;
        v_ib_rate := 0;
      ELSE
        v_fee_pct := COALESCE(get_investor_fee_pct(v_alloc.investor_id, p_fund_id, v_period_end), 0);
        -- FIX: Use ib_parent_id for IB commission lookup, not investor_id
        IF v_alloc.ib_parent_id IS NOT NULL THEN
          v_ib_rate := COALESCE(get_investor_ib_pct(v_alloc.ib_parent_id, p_fund_id, v_period_end), 0);
        ELSE
          v_ib_rate := 0;
        END IF;
      END IF;
  ...
  FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
    ...
    -- FIX: Also skip fee crediting for ib accounts (they don't pay fees)
    IF v_alloc.fee > 0 AND v_alloc.account_type NOT IN ('fees_account', 'ib') THEN
    ...
    IF v_alloc.ib > 0 AND v_alloc.ib_parent_id IS NOT NULL AND v_alloc.account_type NOT IN ('fees_account', 'ib') THEN
  ...
  IF v_residual <> 0 AND v_fees_account_id IS NOT NULL THEN
    ...
    UPDATE transactions_v2 SET visibility_scope = 'admin_only'::visibility_scope
    WHERE id = (v_tx_result->>'tx_id')::uuid;
  END IF;
...
VALUES ('4ca7a856-dbd1-4b75-8092-eb0c6070ba55', '7574bc81-aab3-4175-9e7f-803aa6f9eb8f', 4.00, '2024-01-01')
ON CONFLICT DO NOTHING;

-- Also ensure Paul Johnson has proper fee schedule in SOL (16% INDIGO fee)
-- He currently only has 13.5% via BTC fund fallback
INSERT INTO investor_fee_schedule (investor_id, fund_id, fee_pct, effective_date)
VALUES ('96fbdf46-cd6a-4f99-8a0d-11607f051ccd', '7574bc81-aab3-4175-9e7f-803aa6f9eb8f', 16.00, '2024-01-01')
ON CONFLICT DO NOTHING;
