-- Fix yield_distributions schema usage:
-- - is_voided -> voided_at IS NOT NULL
-- - distribution_date -> effective_date
-- - Remove non-existent columns (opening_balance, closing_balance, investor_id, period_id)
...
BEGIN
  ...
  -- If no prior AUM event, use sum of current investor positions
  ...
  LOOP
    ...
    -- Check if this yield was already distributed (use voided_at IS NULL instead of is_voided = false)
    SELECT EXISTS(
...
BEGIN
  ...
  LOOP
    ...
    -- Check if already distributed (idempotency) - use voided_at IS NULL
    ...
    -- Create yield distribution record (fund-level, not per-investor)
    -- We create one record per yield run, storing summary_json for details
    IF v_distribution_id IS NULL THEN
      INSERT INTO yield_distributions (
        ...
        v_gross_yield, 0, 0, 0,  -- will update totals at end
