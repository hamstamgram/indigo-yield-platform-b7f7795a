-- Fixing IB Exclusion & Zero-Transaction Exclusion in Statement Pre-Generation
-- 1. Extend populate_investor_fund_performance to include BOTH 'investor' and 'ib' accounts.
-- 2. Modify get_reporting_eligible_investors to include 'ib' accounts.
...
BEGIN
  ...
  FOR r IN 
    ...
    WHERE ip.current_value > 0 -- Optimization to only run this for active ledgers
      AND p.account_type IN ('investor', 'ib') -- ALIGNMENT FIX 1: Ensure IB tracks statements!
      AND (p_investor_id IS NULL OR ip.investor_id = p_investor_id)
  LOOP
    
    -- ALIGNMENT FIX 2: Do not generate empty statements for accounts that had truly zero balance and activity during this specific month
    IF r.exact_ending_balance <= 0 AND r.mtd_beginning_balance <= 0 AND r.mtd_additions = 0 AND r.mtd_redemptions = 0 AND r.mtd_net_income = 0 THEN
      CONTINUE; 
    END IF;
...
CREATE OR REPLACE FUNCTION public.get_reporting_eligible_investors(p_period_id uuid)
 RETURNS TABLE(investor_id uuid, investor_name text, email text, eligibility_reason text, is_eligible boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
    ...
    WITH investor_data AS (
        ...
        FROM profiles p
        -- ALIGNMENT FIX 3: Ensure IBs are considered eligible for reporting checks
        WHERE p.account_type IN ('investor', 'ib') OR p.account_type IS NULL
    )
    SELECT 
        id.inv_id,
        id.inv_name,
