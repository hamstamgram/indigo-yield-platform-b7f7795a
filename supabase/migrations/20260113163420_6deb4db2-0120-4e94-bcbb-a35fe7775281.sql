-- Drop and recreate get_void_yield_impact with correct column references
DROP FUNCTION IF EXISTS public.get_void_yield_impact(uuid);

CREATE FUNCTION public.get_void_yield_impact(p_distribution_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_tx_count INTEGER;
BEGIN
  -- Get distribution details (using 'status' not 'is_voided')
  ...
  -- Count affected investors
  SELECT COUNT(DISTINCT investor_id), COALESCE(SUM(amount), 0)
  INTO v_affected_count, v_total_yield
  FROM transactions_v2
  WHERE yield_distribution_id = p_distribution_id AND is_voided = false AND type = 'YIELD';
  
  SELECT COUNT(*) INTO v_tx_count
  FROM transactions_v2 WHERE yield_distribution_id = p_distribution_id AND is_voided = false;
  
  SELECT COALESCE(SUM(fee_amount), 0) INTO v_total_fees
  FROM fee_allocations WHERE distribution_id = p_distribution_id AND is_voided = false;
  
  SELECT COALESCE(SUM(ib_fee_amount), 0) INTO v_total_ib
  FROM ib_allocations WHERE distribution_id = p_distribution_id AND is_voided = false;
  ...
  SELECT json_build_object(
    ...
    'investors', COALESCE((
      ...
      LEFT JOIN fee_allocations f ON f.distribution_id = p_distribution_id AND f.investor_id = t.investor_id AND f.is_voided = false
      WHERE t.yield_distribution_id = p_distribution_id AND t.is_voided = false AND t.type = 'YIELD'
    ), '[]'::json)
  ) INTO v_result;
