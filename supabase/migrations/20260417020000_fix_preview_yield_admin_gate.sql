-- =============================================================
-- Fix yield preview: Restore baseline preview_segmented_yield_distribution_v5
-- 2026-04-17 | HIGH: The preview function was NOT replaced by Tier 1
-- but it was also missing an admin gate. This migration adds the
-- missing is_admin() gate and canonical_rpc flag, and delegates to
-- calculate_yield_allocations which now works correctly since
-- can_access_investor() has the canonical_rpc bypass (migration 20260417000000).
-- =============================================================
...
BEGIN
  -- Admin gate (was missing)
  IF NOT public.is_admin() THEN
    RAISE EXCEPTION 'Unauthorized: admin role required (JWT session check)';
  END IF;

  -- Set canonical_rpc so can_access_investor() bypass works for
  -- get_investor_fee_pct / get_investor_ib_pct called by
