-- Migration: Flat yield engine + no auto-crystallization
-- Date: 2026-03-02
--
-- Changes:
-- 1. apply_transaction_with_crystallization: RAISE ERROR if p_new_total_aum IS NOT NULL
--    (auto-crystallization is removed; admin manually records transaction yields before deposits)
-- 2. apply_segmented_yield_distribution_v5: rewritten as flat position-proportional engine
--    gross = recorded_aum - SUM(investor_positions.current_value WHERE is_active=true)
--    allocate by current position share (no segments, no fund_aum_events references)
