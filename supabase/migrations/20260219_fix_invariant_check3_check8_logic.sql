-- Fix run_invariant_checks(): check 3 and check 8 logic corrections.
--
-- CHECK 3 (yield_conservation): Added AND yd.purpose = 'reporting'::aum_purpose
--   Transaction-purpose (checkpoint) distributions intentionally omit fee_allocations
--   and ib_allocations. Conservation identity (gross = net+fees+ib) only applies to
--   reporting distributions that fire real YIELD/FEE_CREDIT/IB_CREDIT transactions.
--
-- CHECK 8 (ib_allocation_count_matches): Changed from profiles.ib_percentage > 0
--   (current profile state) to ya.ib_pct > 0 (rate at distribution time).
--   If a profile's ib_percentage changed to 0 after a distribution was created,
--   the old logic would report expected_count=0 but ib_alloc_count=1, a false positive.
--   Using ya.ib_pct from yield_allocations reflects the historical state correctly.
--
-- NOTE: This migration is superseded by 20260219_fix_invariant_check8_reporting_scope
--   which also scopes check 8 to purpose='reporting' only. The final version of the
--   function is in that migration.
...
BEGIN
  ...
  WITH latest_aum AS (
    ...
    FROM fund_daily_aum WHERE is_voided = false AND purpose = 'transaction'
  ...
  aum_check AS (
    ...
    WHERE a.aum_date = CURRENT_DATE AND ABS(a.total_aum - COALESCE(p.position_sum, 0)) > 0.01
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'fund_id', fund_id, 'aum_value', aum_value, 'position_sum', position_sum, 'drift', drift, 'aum_date', aum_date
  )), '[]'::jsonb), COUNT(*) INTO v_violations, v_violation_count FROM aum_check;
  ...
  -- Fixed: only check REPORTING distributions. TRANSACTION (checkpoint) distributions
  -- intentionally omit fee_allocations and ib_allocations, so gross != net+fees+ib for them.
  ...
  -- Fixed: use ya.ib_pct > 0 from yield_allocations (rate at distribution time),
  -- not current profiles.ib_percentage (which may have changed since distribution).
  WITH ib_count AS (
  ...
  WITH orphans AS (
    ...
    FROM auth.users au LEFT JOIN profiles p ON p.id = au.id WHERE p.id IS NULL
  )
