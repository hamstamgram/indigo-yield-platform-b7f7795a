...
-- Fix: Full exit dust sweep should respect p_processed_amount when provided
-- Previously, full exit ALWAYS auto-computed via TRUNC(balance, precision), ignoring admin input.
-- Now: if admin provides p_processed_amount + full_exit=true, use that amount and sweep remainder as dust.
...
BEGIN
  ...
  IF p_is_full_exit THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Crystallization may fail if no yield to distribute; that is OK
      NULL;
    END;
  END IF;
  ...
  IF p_is_full_exit THEN
    -- If admin provided a specific processed amount, use it and sweep the remainder as dust.
    -- Otherwise, auto-truncate the balance to send precision (legacy behavior).
    ...
    -- Safety: if final amount exceeds balance, cap it
