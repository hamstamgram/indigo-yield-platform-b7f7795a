-- Fix update_investor_aum_percentages to use UUID parameter instead of TEXT
CREATE OR REPLACE FUNCTION public.update_investor_aum_percentages(p_fund_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Fix force_delete_investor: remove TEXT cast and add missing platform_fees_collected delete
...
BEGIN
  ...
  DELETE FROM platform_fees_collected WHERE investor_id = p_investor_id;
  ...
  -- Recalculate AUM percentages for affected funds (now using UUID directly)
