-- Migration: Add snapshot_time to yield_distributions + allow multiple transaction-purpose distributions per day
--
-- Problem: Transaction-purpose yield was blocked to one-per-day by duplicate check in apply_adb_yield_distribution_v4.
-- This forced admins to void-and-replace when AUM changed intraday (e.g., new deposit arrived).
--
-- Solution: For transaction purpose, allow multiple distributions per day (each is an incremental yield event).
-- Add snapshot_time (HH:MM) for audit trail. Keep reporting purpose locked to one-per-period.

-- 1. Add snapshot_time column
ALTER TABLE yield_distributions
  ADD COLUMN IF NOT EXISTS snapshot_time TEXT;

COMMENT ON COLUMN yield_distributions.snapshot_time
  IS 'HH:MM when admin recorded the AUM snapshot. Transaction purpose only. Audit trail.';

-- 2. Recreate apply_adb_yield_distribution_v4 with:
--    a) New p_snapshot_time parameter
--    b) Duplicate check only for reporting purpose (transaction purpose allows multiple per day)
--    c) snapshot_time stored in yield_distributions INSERT
