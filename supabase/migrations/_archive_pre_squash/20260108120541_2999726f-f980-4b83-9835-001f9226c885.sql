-- Fix fee lookup: include global fees (fund_id IS NULL) and profiles.fee_pct, remove 20% fallback

-- Step 1: Fix INDIGO LP fee schedule - remove end_date
UPDATE investor_fee_schedule 
SET end_date = NULL
WHERE investor_id = 'ec182081-9bbb-4624-84ac-41b086985d8c'
  AND fee_pct = 0;

-- Step 2: Recreate yield functions with corrected fee lookup
...
-- ============================================
-- preview_daily_yield_to_fund_v3 - Fixed fee lookup
-- ============================================
...
BEGIN
  ...
  -- FIX: Use hierarchical fee lookup: fund-specific > global > profiles.fee_pct > 0
  FOR v_inv IN
    SELECT 
      ip.investor_id,
      ...
      COALESCE(
        ...
        (SELECT ifs1.fee_pct / 100 
         ...
         LIMIT 1),
        -- Priority 2: Global fee schedule (fund_id IS NULL)
        (SELECT ifs2.fee_pct / 100 
         FROM investor_fee_schedule ifs2
         WHERE ifs2.investor_id = ip.investor_id
         ...
         LIMIT 1),
        -- Priority 3: Profile fee_pct
        p.fee_pct / 100,
        -- Priority 4: Zero (no arbitrary 20% fallback)
        0
      ) as fee_pct,
      p.ib_parent_id,
  ...
  LOOP
    ...
    -- Get fee percentage (already converted to decimal)
    ...
    -- Add IB credit record if applicable
    IF v_ib_parent_id IS NOT NULL AND v_ib_pct > 0 THEN
...
-- ============================================
-- apply_daily_yield_to_fund_v3 - Fixed fee lookup
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    -- Create yield distribution record (fund-level, only once)
    IF v_distribution_id IS NULL THEN
      INSERT INTO yield_distributions (
        fund_id, effective_date, purpose, is_month_end,
        opening_aum, closing_aum, recorded_aum, previous_aum,
        gross_yield, net_yield, total_fees, total_ib,
        yield_percentage, reference_id, period_start, period_end,
        status, created_by
      ) VALUES (
        p_fund_id, p_yield_date, p_purpose::aum_purpose, v_is_month_end,
        v_opening_aum, p_new_aum, p_new_aum, v_opening_aum,
        v_gross_yield, 0, 0, 0,
        v_yield_pct, 'yield_' || v_fund_code || '_' || p_yield_date::text, p_yield_date, p_yield_date,
        'applied', p_actor_id
    ...
    IF v_ib_parent_id IS NOT NULL AND v_ib_pct > 0 THEN
      ...
      INSERT INTO ib_allocations (
        ...
        effective_date, purpose, source, created_by
      ) VALUES (
        v_distribution_id, p_fund_id, v_ib_parent_id, v_inv.investor_id,
        v_inv_gross, v_inv.ib_pct_raw, v_ib_amount,
        p_yield_date, p_purpose::aum_purpose, 'yield_fee', p_actor_id
      );
  ...
  -- Update yield distribution with final totals
  ...
  -- Create INDIGO fees credit transaction if applicable
  IF v_indigo_fees_credit > 0 AND v_indigo_fees_id IS NOT NULL THEN
    INSERT INTO transactions_v2 (
      ...
      v_fund_asset, v_fund_class, 'yield_distribution', 'admin_only', true,
      'Platform fees from yield distribution', p_actor_id
    );
    
    -- Update INDIGO fees position
    INSERT INTO investor_positions (investor_id, fund_id, current_value)
    VALUES (v_indigo_fees_id, p_fund_id, v_indigo_fees_credit)
...
GRANT EXECUTE ON FUNCTION public.apply_daily_yield_to_fund_v3(uuid, date, numeric, uuid, text) TO authenticated;