-- Fix preview_daily_yield_to_fund_v2 to use correct column names:
-- 1. investor_positions.current_value (not current_balance)
-- 2. profiles.ib_percentage (not ib_commission_pct)
...
BEGIN
  ...
  FOR v_investor IN
    SELECT 
      ip.investor_id,
      ip.current_value as position_balance,  -- FIXED: was current_balance
      p.first_name,
      p.last_name,
      p.ib_parent_id,
      COALESCE(
      ...
      COALESCE(p.ib_percentage, 0) as ib_pct  -- FIXED: was ib_commission_pct
    FROM investor_positions ip
    JOIN profiles p ON p.id = ip.investor_id
    WHERE ip.fund_id = p_fund_id
      AND ip.current_value > 0  -- FIXED: was current_balance
  ...
  LOOP
    ...
    v_investor_ib_pct := v_investor.ib_pct;  -- FIXED: was ib_commission_pct
...
-- Also fix apply_daily_yield_to_fund_v2 to use correct column name
...
BEGIN
  ...
  LOOP
    ...
    -- Update investor position with net yield - FIXED: use current_value not current_balance
