-- Fix global fee schedule lookup in yield functions
-- Problem: Current JOIN only matches fund_id = p_fund_id, missing global fees (fund_id IS NULL)
-- Solution: Use subquery with priority: fund-specific > global > default 20%

CREATE OR REPLACE FUNCTION public.preview_daily_yield_to_fund_v3(p_fund_id uuid, p_yield_date date, p_new_aum numeric, p_purpose text DEFAULT 'reporting'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
