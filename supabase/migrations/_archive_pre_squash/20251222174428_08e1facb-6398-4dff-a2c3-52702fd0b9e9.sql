-- Fix: Update apply_daily_yield_to_fund_v2 to insert into yield_distributions table
-- This ensures proper audit trail for all yield distributions

CREATE OR REPLACE FUNCTION public.apply_daily_yield_to_fund_v2(p_fund_id uuid, p_date date, p_gross_amount numeric, p_admin_id uuid, p_purpose text DEFAULT 'transaction'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  END LOOP;
  IF v_total_fees > 0 THEN v_fee_ref := format('fee_credit:%s:%s:%s', p_fund_id, p_date, v_purpose_enum); IF NOT EXISTS (SELECT 1 FROM transactions_v2 WHERE reference_id = v_fee_ref) THEN INSERT INTO transactions_v2 (id, investor_id, fund_id, type, asset, fund_class, amount, tx_date, reference_id, notes, created_by, created_at, purpose, distribution_id) VALUES (gen_random_uuid(), v_indigo_fees_id, p_fund_id, 'FEE_CREDIT', v_asset, v_fund.fund_class, v_total_fees, p_date, v_fee_ref, format('Platform fees collected (after IB: %s) - %s', ROUND(v_total_ib_fees::numeric, 8), v_purpose_enum), p_admin_id, now(), v_purpose_enum, v_distribution_id); UPDATE investor_positions SET current_value = current_value + v_total_fees, updated_at = now() WHERE investor_id = v_indigo_fees_id AND fund_id = p_fund_id; IF NOT FOUND THEN INSERT INTO investor_positions (investor_id, fund_id, fund_class, current_value, cost_basis, shares) VALUES (v_indigo_fees_id, p_fund_id, v_fund.fund_class, v_total_fees, 0, 0); END IF; END IF; END IF;
  ...
  -- FIX: Insert into yield_distributions for proper audit trail
  INSERT INTO yield_distributions (
    id, fund_id, effective_date, purpose, is_month_end,
    recorded_aum, gross_yield, distribution_type,
