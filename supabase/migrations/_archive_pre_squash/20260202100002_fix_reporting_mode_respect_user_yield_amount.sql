-- Fix: Reporting mode unconditionally overrides user-provided gross yield with
-- SUM of existing YIELD transactions. When no YIELD transactions exist, yield = 0
-- and the distribution is empty.
--
-- Fix: Only derive from existing YIELD transactions when user provides 0 or NULL.
-- When user provides a positive p_gross_yield_amount (from AUM input), use it.
-- Also remove the hard "no transactions found" check — reporting mode should work
-- for initial yield recording too, not just re-recording existing yields.
...
BEGIN
  ...
  -- Reporting mode: derive period, optionally derive yield from transactions
  IF p_purpose = 'reporting'::aum_purpose THEN
    ...
    -- FIX: Only derive yield from existing transactions when user provides 0 or NULL
    -- When user provides a positive amount (from AUM input), respect it
    IF COALESCE(p_gross_yield_amount, 0) <= 0 THEN
      SELECT COALESCE(SUM(amount), 0)
      INTO v_derived_yield
      FROM transactions_v2
      WHERE fund_id = p_fund_id
        AND is_voided = false
  ...
  LOOP
    ...
    INSERT INTO yield_allocations (
      ...
      adb_share, fee_pct, ib_pct, transaction_id, created_at
    ) VALUES (
      v_distribution_id, v_investor.investor_id, p_fund_id,
      v_gross_share, v_net_share, v_fee_share, v_ib_share,
      v_investor.adb, v_investor.fee_pct, v_investor.ib_rate, v_yield_tx_id, NOW()
    );
    ...
    IF v_fee_share > 0 THEN
      INSERT INTO fee_allocations (
        ...
        credit_transaction_id, created_by
      ) VALUES (
        v_distribution_id, p_fund_id, v_investor.investor_id, v_fees_account_id,
        v_period_start, v_period_end, p_purpose,
        v_gross_share, v_investor.fee_pct, v_fee_share,
        NULL, v_admin
      );
    END IF;
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO fund_daily_aum (
      id, fund_id, aum_date, total_aum, as_of_date,
      is_month_end, purpose, source, created_at, created_by, is_voided
    ) VALUES (
      gen_random_uuid(), p_fund_id, v_period_end,
      v_current_aum + v_gross_yield_amount, v_period_end,
      v_is_month_end, p_purpose, 'yield_distribution',
      NOW(), v_admin, false
    );
  END IF;

  INSERT INTO audit_log (
    actor_user, action, entity, entity_id, old_values, new_values, meta
  ) VALUES (
    v_admin,
