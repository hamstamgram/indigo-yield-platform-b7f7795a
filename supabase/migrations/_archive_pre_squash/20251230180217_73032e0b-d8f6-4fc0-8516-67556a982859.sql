-- Fix yield distribution functions: correct fee/IB interpretation and add shares update
-- Issue 1: ib_percentage is stored as whole number (10 = 10%), not decimal
-- Issue 2: shares column not being updated, causing position/ledger divergence
-- Issue 3: Apply function doesn't match preview function logic
...
-- Recreate preview function with correct logic
...
BEGIN
  ...
  -- Get current AUM from investor positions (excluding INDIGO FEES account)
  ...
  -- Loop through each investor with a position (excluding INDIGO FEES)
  FOR v_investor IN
    SELECT 
      ip.investor_id,
      ...
      p.account_type,
      -- Fee resolution: investor_fee_schedule (whole %) > profiles.fee_percentage (decimal) > 20%
      COALESCE(
        (SELECT fee_pct FROM investor_fee_schedule ifs 
         ...
         LIMIT 1),
        COALESCE(p.fee_percentage * 100, 20)  -- fee_percentage is decimal: 0.20 = 20%
      ) as fee_pct,
      p.ib_parent_id,
      -- ib_percentage is already whole number: 10 = 10%
...
-- Recreate apply function using preview data and updating shares
...
BEGIN
  ...
  IF v_conservation_check > 0.01 THEN
    RETURN jsonb_build_object(
      ...
      'error', format('Conservation check failed: gross (%.8f) != net (%.8f) + fees (%.8f), diff: %.8f',
        p_gross_yield, (v_preview->>'total_net')::numeric, (v_preview->>'total_fees')::numeric, v_conservation_check)
    );
  END IF;
  ...
  LOOP
    ...
    -- Record fee deduction (for audit trail, fee not subtracted again since net_yield already accounts for it)
...
-- Add comment documenting the fee storage formats
COMMENT ON FUNCTION public.preview_daily_yield_to_fund_v2 IS 'Preview yield distribution. Fee sources: investor_fee_schedule.fee_pct (whole %), profiles.fee_percentage (decimal, e.g. 0.20 = 20%), profiles.ib_percentage (whole %)';
COMMENT ON FUNCTION public.apply_daily_yield_to_fund_v2 IS 'Apply yield distribution. Uses preview function for calculations. Updates both current_value and shares columns.';