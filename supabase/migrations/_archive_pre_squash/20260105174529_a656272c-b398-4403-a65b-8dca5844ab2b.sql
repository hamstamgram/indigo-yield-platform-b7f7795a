...
-- ib_percentage is stored as 2.00 meaning 2%, not 0.02
-- fee_pct from investor_fee_schedule is stored as 0.20 meaning 20%, not 20
-- Need to normalize: ib_percentage / 100, fee_pct is already decimal
...
BEGIN
  ...
  -- IMPORTANT: Use sum of current investor positions as opening AUM
  -- This ensures we're calculating yield based on actual current holdings
  ...
  FOR v_inv IN
    SELECT 
      ...
      COALESCE(ifs.fee_pct, 0.20) as fee_pct,  -- fee_pct is stored as decimal (0.20 = 20%)
      p.ib_parent_id,
      COALESCE(p.ib_percentage, 0) as ib_pct_raw  -- ib_percentage stored as whole number (2 = 2%)
  ...
  LOOP
    ...
    -- Get fee percentage (already stored as decimal)
    ...
    -- FIXED: Convert ib_percentage from whole number to decimal (2 -> 0.02)
    ...
    IF v_ib_parent_id IS NOT NULL AND v_ib_pct > 0 THEN
      ...
      v_ib_credits := v_ib_credits || jsonb_build_object(
        ...
        'amount', v_ib_amount,
        'ibPercentage', v_inv.ib_pct_raw,  -- Display as whole number (2%)
        'source', 'yield_fee',
    ...
    v_distributions := v_distributions || jsonb_build_object(
      ...
      'feePercentage', v_fee_pct * 100,  -- Display as whole number (20%)
      ...
      'ibPercentage', v_inv.ib_pct_raw,  -- Display as whole number
...
BEGIN
  ...
  LOOP
    ...
    -- Get fee percentage (stored as decimal)
    v_fee_pct := v_inv.fee_pct;
    ...
    -- Create yield distribution record (fund-level)
    ...
    -- Handle IB commission if applicable
    v_ib_parent_id := v_inv.ib_parent_id;
    -- FIXED: Convert ib_percentage from whole number to decimal
    v_ib_pct := v_inv.ib_pct_raw / 100;
    v_ib_amount := 0;
    v_ib_parent_name := NULL;
    ...
    IF v_ib_parent_id IS NOT NULL AND v_ib_pct > 0 THEN
      ...
      INSERT INTO ib_allocations (
        ...
        v_inv_fee, v_inv.ib_pct_raw, v_ib_amount,
  ...
  -- Update yield_distributions with final totals
