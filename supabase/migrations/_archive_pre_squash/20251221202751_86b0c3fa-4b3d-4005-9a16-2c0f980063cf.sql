-- P0 Fix: FEE_CREDIT visibility enforcement and force_delete_investor search_path

-- 1. Fix enforce_internal_tx_visibility to include FEE_CREDIT
...
BEGIN
  ...
  -- These should NEVER be visible to investors
...
-- 2. Fix existing FEE_CREDIT transactions to be admin_only
UPDATE transactions_v2 
SET visibility_scope = 'admin_only' 
WHERE type = 'FEE_CREDIT' AND visibility_scope = 'investor_visible';

-- 3. Fix force_delete_investor search_path (remove extensions from path)
-- First drop and recreate with correct search_path
...
BEGIN
  ...
  -- ============================================
  -- DELETE FROM ALL RELATED TABLES (NOT VIEWS!)
  -- ============================================
  ...
  -- Fee related tables (NOT platform_fees_collected - that's a VIEW!)
  ...
  IF v_fund_ids IS NOT NULL THEN
    FOREACH v_fund_id IN ARRAY v_fund_ids LOOP
      PERFORM update_investor_aum_percentages(v_fund_id);
    END LOOP;
  END IF;
...
-- 4. Log the security fix in audit
INSERT INTO audit_log (action, entity, entity_id, actor_user, meta)
VALUES (
  'SECURITY_FIX',
  'system',
  ...
  jsonb_build_object(
    'fixes_applied', ARRAY[
      'FEE_CREDIT visibility enforcement trigger updated',
      'Existing FEE_CREDIT transactions set to admin_only',
      'force_delete_investor search_path fixed'
    ],
    'applied_at', now()
  )
);