-- ============================================================
-- Consolidate profiles triggers: extend existing
-- protect_profile_sensitive_fields to cover the additional
-- columns that enforce_profiles_safe_columns was guarding,
-- then drop the redundant enforce_profiles_safe_columns.
--
-- Columns added to existing trigger:
--   fee_pct, ib_percentage, totp_enabled, totp_verified
--   (ib_parent_id already gated by existing trigger for non-admins)
-- ============================================================
...
BEGIN
  ...
  IF NEW.totp_enabled IS DISTINCT FROM OLD.totp_enabled THEN
    RAISE EXCEPTION 'Cannot modify totp_enabled field';
  END IF;
  IF NEW.totp_verified IS DISTINCT FROM OLD.totp_verified THEN
    RAISE EXCEPTION 'Cannot modify totp_verified field';
  END IF;
...
-- Drop the redundant trigger and its function
DROP TRIGGER IF EXISTS enforce_profiles_safe_columns ON public.profiles;
DROP FUNCTION IF EXISTS public.enforce_profiles_safe_columns();