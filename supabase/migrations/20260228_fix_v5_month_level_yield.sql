-- Migration: Fix V5 to use month-level yield computation
-- Date: 2026-02-28
--
-- ROOT CAUSE: V5 computed yield per-segment and skipped negative segments.
-- For months with mixed positive/negative segments, V5 distributed only the
-- positive segments' yield, OVER-distributing compared to the correct monthly total.
-- Example: BTC Dec 2024 total = +0.109, but V5 distributed +0.233 (skipped -0.124 segment).
-- This caused BTC total V5 gross = 9.12 vs Excel 1.56 (484% over).
--
-- FIX: Compute yield at MONTH level: total_yield = pClosing - opening - flows.
-- This guarantees positions track pClosing after each month (self-correcting).
-- Negative yield: proportional loss, no fees/IB (per scorched earth rules).
-- Crystal markers still consolidated for bookkeeping but not used for yield computation.
...
BEGIN
  ...
  -- Includes ALL investors with any transactions in the fund
  -- ============================================================
  ...
  -- This guarantees positions = pClosing after distribution
  -- ============================================================
  ...
  END LOOP;
  -- Count includes the final segment
  v_seg_count := v_seg_count + 1;
  ...
  -- ============================================================
  -- YIELD ALLOCATION
  -- Positive yield: proportional to opening_balance, apply fees/IB
  -- Negative yield: proportional loss, no fees/IB (scorched earth)
  -- Zero yield: no allocation
  -- ============================================================
  IF v_total_month_yield != 0 AND v_opening_balance_sum > 0 THEN
    ...
    LOOP
      ...
      -- Track fees_account gross separately (excluded from header)
      ...
      -- Ensure fees_account and IB parent exist in _v5_tot
  ...
  -- Dust = month_yield - investor_gross - fees_account_gross
  -- ============================================================
  ...
  LOOP
    -- YIELD transaction (positive or negative, but not zero)
    v_yield_tx_id := NULL;
    ...
    -- yield_allocations record (always created when there is a gross allocation)
    ...
    -- Fee allocation records (only for positive yield, skip fees_account)
    ...
    -- IB allocation (only for positive yield)
  ...
  -- ============================================================
  -- FEE_CREDIT to fees_account (only for positive yield)
  -- ============================================================
