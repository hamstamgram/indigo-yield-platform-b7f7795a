-- P1: Add missing investor_id index (without CONCURRENTLY for transaction compatibility)
CREATE INDEX IF NOT EXISTS idx_withdrawal_requests_investor 
ON withdrawal_requests(investor_id);

-- Update the complete_withdrawal function to create ledger entries
...
COMMENT ON FUNCTION public.complete_withdrawal IS 
'Completes a withdrawal request by creating a WITHDRAWAL transaction in the ledger, 
updating investor positions, and marking the request as completed. 
Idempotent via reference_id check. Token-denominated only.';