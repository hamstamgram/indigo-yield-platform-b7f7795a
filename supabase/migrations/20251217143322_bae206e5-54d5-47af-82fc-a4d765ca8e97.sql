-- Fix get_funds_with_aum to prioritize live investor_positions data over stale fund_daily_aum
...
BEGIN
    RETURN QUERY
    WITH live_aum AS (
        -- PRIMARY: Get live AUM from investor_positions (source of truth)
        SELECT
            ip.fund_id,
    ...
    historical_aum AS (
        -- FALLBACK ONLY: Use fund_daily_aum if no investor positions exist
    ...
    SELECT
        ...
        f.inception_date,
        f.status,
        -- PRIORITY: live_aum first, then historical_aum as fallback
