-- Fix void_transaction: Remove invalid updated_at column reference
-- transactions_v2 does not have an updated_at column
...
BEGIN
  ...
  -- Calculate projected balance (no FIRST_INVESTMENT - it's not a valid enum)
  SELECT COALESCE(SUM(
    CASE 
  ...
  -- Update without updated_at (column does not exist)
