-- Migration: V4 Yield RPC Enrichment of investor_yield_events
-- Date: 2026-02-11
--
-- Problem: sync_yield_to_investor_yield_events trigger creates rows with hardcoded zeros
-- for investor_balance, investor_share_pct, fund_yield_pct, fee_pct, fee_amount.
-- The trigger fires on YIELD tx INSERT but doesn't have access to allocation details.
--
-- Fix: Add UPDATE step in apply_adb_yield_distribution_v4 after yield_allocations INSERT
-- to enrich the investor_yield_events rows with computed allocation data.
-- Also fixes gross_yield_amount (trigger sets it to net tx amount, should be gross).
...
BEGIN
  ...
  LOOP
    ...
    -- The sync_yield_to_investor_yield_events trigger fires on the
    -- YIELD tx INSERT above, creating a row with hardcoded zeros.
    -- Now enrich it with actual allocation data.
    -- ================================================================
  ...
  IF v_residual != 0 AND v_largest_alloc_investor_id IS NOT NULL THEN
    ...
    -- Also update the enriched investor_yield_events for the adjusted investor
    UPDATE investor_yield_events
