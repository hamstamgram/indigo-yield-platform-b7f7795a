-- Fix false positive in run_invariant_checks: fund_aum_matches_positions (check 2)
--
-- Three bugs fixed:
-- 1. No purpose filter: DISTINCT ON picked any AUM record (transaction or reporting)
--    but positions should only be compared against transaction-purpose AUM
-- 2. No is_active filter: summed ALL positions including inactive/zero-balance,
--    while AUM recording functions filter is_active=true / current_value>0
-- 3. Stale AUM comparison: compared historical AUM snapshot (e.g., Dec 31) against
--    live positions that have changed since then. This is expected temporal divergence,
--    not a data integrity violation.
--
-- Fix: filter purpose='transaction', is_active=true, and only flag mismatches
-- when the latest AUM record is from today (same-day comparison only).
