-- Fix: apply_segmented_yield_distribution_v5 calls apply_investor_transaction with
-- p_tx_type := 'YIELD'::tx_type (and 'FEE_CREDIT'::tx_type, 'IB_CREDIT'::tx_type, 'DUST'::tx_type)
-- but apply_investor_transaction expects p_tx_type text, not tx_type.
-- The explicit ::tx_type cast causes PostgreSQL to not match the function signature.
-- Remove the casts so the text literals match text parameter type.

-- Also fix: apply_investor_transaction returns jsonb, but the calling code
-- expects json (uses ->>'transaction_id'). The function was updated in baseline
-- to return jsonb, but the callers still store into v_tx_result json.
-- This is fine because jsonb -> json assignment works transparently in PL/pgSQL.
...
CREATE OR REPLACE FUNCTION public.apply_segmented_yield_distribution_v5(
  ...
  p_opening_aum numeric DEFAULT NULL::numeric
) RETURNS jsonb
...
BEGIN
  ...
  INSERT INTO yield_distributions (
    ...
    p_fund_id, v_period_end, v_tx_date, v_period_start, v_period_end,
