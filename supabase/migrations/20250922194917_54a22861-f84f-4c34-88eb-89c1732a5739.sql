-- PHASE 1: Complete Platform Remediation - Database Schema Normalization & Security
...
-- =============================================
-- Step 5: Fix Asset and Portfolio Architecture
-- =============================================

-- Ensure assets_v2 has all necessary data
INSERT INTO public.assets_v2 (asset_id, symbol, name, kind, decimals, is_active)
SELECT 
    UPPER(a.symbol::text) as asset_id,
    a.symbol::text,
    a.name,
    'cryptocurrency' as kind,
    a.decimal_places,
    a.is_active
...
-- Ensure investors table is properly populated
...
-- =============================================
-- Step 6: Create Sample Fund Data
-- =============================================

-- Insert default funds if they don't exist
INSERT INTO public.funds (id, code, name, asset, fund_class, status, inception_date)
VALUES 
    (gen_random_uuid(), 'BTC-A', 'Bitcoin Fund Class A', 'BTC', 'Class A', 'active', CURRENT_DATE),
    (gen_random_uuid(), 'ETH-A', 'Ethereum Fund Class A', 'ETH', 'Class A', 'active', CURRENT_DATE),
    (gen_random_uuid(), 'USDT-A', 'Tether Fund Class A', 'USDT', 'Class A', 'active', CURRENT_DATE)
ON CONFLICT (code) DO NOTHING;

-- =============================================
-- Step 7: Create Investor Positions from Existing Positions
-- =============================================

-- Create investor positions based on existing positions
INSERT INTO public.investor_positions (investor_id, fund_id, shares, cost_basis, current_value, fund_class)
SELECT 
    i.id as investor_id,
    f.id as fund_id,
    ...
    f.fund_class
FROM public.positions p
JOIN public.investors i ON p.user_id = i.profile_id
JOIN public.funds f ON f.asset = p.asset_code::text
WHERE p.current_balance > 0
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '✅ DATABASE REMEDIATION COMPLETE';
    ...
    RAISE NOTICE '   - Database functions: SECURED';
    RAISE NOTICE '';
    RAISE NOTICE 'Next: Update frontend components to use new schema';
END $$;