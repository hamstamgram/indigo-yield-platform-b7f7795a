-- HOTFIX v8: void_transaction corrected for ACTUAL live schema
-- fund_aum_events is a VIEW (not a table) — cannot UPDATE it
-- fund_daily_aum uses simple is_voided flag (no voided_at/voided_by columns)
-- Removes trigger_reference references (column doesn't exist)
...
BEGIN
  ...
  -- NOTE: fund_aum_events is a VIEW — cannot UPDATE. Skipped.
  ...
  VALUES ('VOID', 'transactions_v2', p_transaction_id::text, p_admin_id,
    ...
    jsonb_build_object('source', 'void_transaction_rpc', 'cascade_v8', true,
