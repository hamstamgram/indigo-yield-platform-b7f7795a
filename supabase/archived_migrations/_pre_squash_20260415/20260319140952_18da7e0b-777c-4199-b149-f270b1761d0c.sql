-- Fix apply_segmented_yield_distribution_v5: replace raw INSERT with safe UPDATE-then-INSERT for transaction purpose AUM
-- Drop redundant trg_sync_yield_to_aum trigger that races with the RPC

-- 1. Drop the redundant trigger that causes double-writes and the duplicate key violation
DROP TRIGGER IF EXISTS trg_sync_yield_to_aum ON yield_distributions;

-- 2. Replace the entire function to fix the AUM write section (lines 357-366)
...
BEGIN
  ...
  FOR v_alloc IN SELECT * FROM _vflat_alloc LOOP
    ...
    SELECT apply_investor_transaction(
      ...
      p_source := 'yield_distribution'::tx_source,
      p_admin_id := v_admin,
      p_notes := 'Yield distribution ' || v_distribution_id,
      p_visibility := CASE WHEN v_alloc.account_type = 'fees_account' THEN 'admin_only'::visibility_scope ELSE 'investor_visible'::visibility_scope END
    ) INTO v_tx_result;
    ...
    INSERT INTO yield_allocations (
      ...
      fee_percentage, ib_percentage, opening_balance,
      created_by, purpose, transaction_id
    ) VALUES (
      ...
      v_admin, p_purpose, v_yield_tx_id
    );
    ...
    IF v_alloc.fee > 0 AND v_alloc.account_type <> 'fees_account' THEN
      SELECT apply_investor_transaction(
        ...
        p_source := 'fee_allocation'::tx_source,
        p_admin_id := v_admin,
        p_notes := 'Fee from ' || v_alloc.investor_name,
        p_visibility := 'admin_only'::visibility_scope
    ...
    IF v_alloc.ib > 0 AND v_alloc.ib_parent_id IS NOT NULL AND v_alloc.account_type <> 'fees_account' THEN
      SELECT apply_investor_transaction(
        ...
        p_source := 'ib_allocation'::tx_source,
        p_admin_id := v_admin,
  ...
  -- Safe UPDATE-then-INSERT for fund_daily_aum (both purposes)
