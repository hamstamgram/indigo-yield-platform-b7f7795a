-- Fix: Withdrawal transactions should use settlement_date, not CURRENT_DATE
-- The settlement_date is set by admin in the CreateWithdrawalDialog date picker.
-- When approved, the resulting ledger transactions should reflect that date.
-- Uses COALESCE for backward compatibility (NULL settlement_date falls back to CURRENT_DATE).
