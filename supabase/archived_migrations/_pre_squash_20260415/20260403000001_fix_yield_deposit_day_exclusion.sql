-- Fix: Exclude same-day deposits/withdrawals from yield allocation
--
-- ROOT CAUSE: When a yield distribution is recorded on the same day as a deposit,
-- the depositor's full balance (including the deposit) is used for yield share
-- calculation. This gives the depositor yield they didn't earn — their money
-- wasn't invested for the period.
--
-- FIX: Compute each investor's "pre-day balance" by subtracting same-day
-- deposit/withdrawal amounts. Use this pre-day balance for:
--   1. v_opening_aum (denominator for share calculation)
--   2. v_share (each investor's proportion)
-- Investors with pre_day_value = 0 (deposited today with no prior balance)
-- get 0% share → 0 yield.
--
-- The total yield (recorded_aum - opening_aum) is unchanged.
-- The recorded_aum includes same-day deposits, so total_yield = closing - opening
-- is the same. Only the ALLOCATION changes: existing investors get 100% of yield,
-- new same-day depositors get 0%.
