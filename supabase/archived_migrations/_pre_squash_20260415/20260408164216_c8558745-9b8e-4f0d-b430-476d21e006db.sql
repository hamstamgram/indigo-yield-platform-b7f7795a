CREATE OR REPLACE FUNCTION "public"."approve_and_complete_withdrawal"("p_request_id" "uuid", "p_processed_amount" numeric DEFAULT NULL::numeric, "p_tx_hash" "text" DEFAULT NULL::"text", "p_admin_notes" "text" DEFAULT NULL::"text", "p_is_full_exit" boolean DEFAULT false, "p_send_precision" integer DEFAULT 18) RETURNS "jsonb"
...
BEGIN
  ...
  IF p_is_full_exit THEN
    -- FIX: Respect p_processed_amount when provided by admin
    IF p_processed_amount IS NOT NULL AND p_processed_amount > 0 THEN
      -- Use admin-entered amount, capped at balance for safety
      v_final_amount := LEAST(p_processed_amount, v_balance);
    ELSE
      -- No amount provided: auto-truncate balance (legacy behavior)
  ...
  -- Unconditionally deactivate position on full exit
  IF p_is_full_exit THEN
