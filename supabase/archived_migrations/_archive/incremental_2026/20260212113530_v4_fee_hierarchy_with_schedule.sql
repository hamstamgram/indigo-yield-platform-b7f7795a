-- Migration: V4 Fee Hierarchy + ADB Period Start Fix
-- Date: 2026-02-12
--
-- Fix 1: Restore get_investor_fee_pct() to full 3-tier hierarchy:
--   1. investor_fee_schedule (fund_id + effective_date match) -> highest priority
--   2. investor_fee_schedule (fund_id IS NULL, global schedule) -> second priority
--   3. profiles.fee_pct (global override) -> third priority
--   4. funds.perf_fee_bps / 100 (fund default) -> fallback
--
-- Fix 2: V4 apply + preview use get_investor_fee_pct() instead of inline COALESCE
--
-- Fix 3: Reporting period_start uses GREATEST(month_start, inception_date)
--   so new funds don't ADB-weight from before their first deposit.

-- ============================================================================
-- FIX 1: Restore get_investor_fee_pct to full hierarchy
-- ============================================================================
...
CREATE OR REPLACE FUNCTION get_investor_fee_pct(
  ...
  SELECT CASE
    ...
    ELSE COALESCE(
      -- Priority 1: Fund-specific fee schedule
      (SELECT ifs.fee_pct FROM investor_fee_schedule ifs
       WHERE ifs.investor_id = p_investor_id
      ...
      -- Priority 2: Global fee schedule (no fund_id)
      (SELECT ifs.fee_pct FROM investor_fee_schedule ifs
       WHERE ifs.investor_id = p_investor_id
         ...
         AND (ifs.end_date IS NULL OR ifs.end_date >= p_effective_date)
       ORDER BY ifs.effective_date DESC LIMIT 1),
      -- Priority 3: Profile-level override
      p.fee_pct,
      -- Priority 4: Fund default (perf_fee_bps / 100)
      (SELECT f.perf_fee_bps::numeric / 100 FROM funds f WHERE f.id = p_fund_id),
      -- Absolute fallback
      0
    )
  END
...
-- ============================================================================
-- FIX 2+3: apply_adb_yield_distribution_v4
-- - Replace inline COALESCE(p.fee_pct, 0) with get_investor_fee_pct()
-- - Adjust reporting period_start to GREATEST(month_start, inception_date)
-- ============================================================================
...
BEGIN
  ...
  -- Load fund early (needed for inception_date in period_start calculation)
  ...
  IF p_purpose = 'reporting'::aum_purpose THEN
    ...
    -- FIX 3: For new funds, start ADB period from inception_date if in same month.
    -- This prevents ADB weighting from days before the fund's first deposit.
    -- Guard: inception_date must be within the distribution month (not a future month).
    IF v_fund.inception_date IS NOT NULL
       AND v_fund.inception_date > v_period_start
       AND v_fund.inception_date <= v_period_end THEN
  ...
  -- Fee + IB overflow guard (uses get_investor_fee_pct for consistency)
  IF EXISTS (
    SELECT 1 FROM investor_positions ip
    JOIN profiles p ON p.id = ip.investor_id
    WHERE ip.fund_id = p_fund_id AND ip.is_active = true
      AND p.account_type = 'investor'
      AND get_investor_fee_pct(ip.investor_id, p_fund_id, v_tx_date) + COALESCE(p.ib_percentage, 0) >= 100
  ) THEN
  ...
  -- Uniqueness check by purpose + exclude voided
  IF EXISTS (
  ...
  FOR v_investor IN
    SELECT
      ...
      -- FIX 2: Use get_investor_fee_pct() for full fee hierarchy
      get_investor_fee_pct(ip.investor_id, p_fund_id, v_tx_date) as fee_pct,
      COALESCE(
  ...
  LOOP
    ...
    -- Enrichment: Update investor_yield_events created by trigger
    IF v_yield_tx_id IS NOT NULL THEN
      ...
      SET
        ...
        investor_share_pct = ROUND((v_investor.adb / v_total_adb * 100)::numeric, 6),
        fund_yield_pct = CASE
          WHEN v_current_aum > 0 THEN ROUND((v_gross_yield_amount / v_current_aum * 100)::numeric, 6)
          ELSE 0
        END,
        gross_yield_amount = v_gross_share,
        fee_pct = v_investor.fee_pct,
        fee_amount = v_fee_share,
        net_yield_amount = v_net_share
  ...
  -- Largest remainder proportional residual adjustment
  ...
  IF v_residual != 0 AND v_largest_alloc_investor_id IS NOT NULL THEN
    ...
    UPDATE investor_yield_events
    SET gross_yield_amount = gross_yield_amount + v_residual,
        fee_amount = fee_amount + v_adj_fee,
        net_yield_amount = net_yield_amount + v_adj_net
    WHERE trigger_transaction_id = (
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'fee_hierarchy', 'investor_fee_schedule > profiles.fee_pct > funds.perf_fee_bps'
    )
  );
  ...
  RETURN jsonb_build_object(
    ...
    'crystals_consolidated', v_crystals_consolidated,
    'features', ARRAY['time_weighted', 'loss_carryforward', 'fee_credit', 'ib_commission', 'latest_period_reporting', 'ib_positions_in_adb', 'ib_fee_exempt', 'fee_allocations', 'audit_log', 'distribution_date', 'fees_account_in_adb', 'ib_auto_payout', 'largest_remainder_proportional', 'satoshi_dust_tolerance', 'fee_hierarchy_3tier', 'admin_recorded_aum', 'fee_ib_overflow_guard', 'purpose_uniqueness', 'reporting_consolidation', 'visibility_scope_control', 'yield_event_enrichment', 'inception_date_period_start']
  );
END;
$$;
...
-- ============================================================================
-- FIX 2+3: preview_adb_yield_distribution_v4
-- - Replace inline COALESCE(p.fee_pct, 0) with get_investor_fee_pct()
-- - Add reporting period_start adjustment for inception_date
-- ============================================================================

CREATE OR REPLACE FUNCTION "public"."preview_adb_yield_distribution_v4"(
  "p_fund_id" "uuid",
  "p_period_start" "date",
  "p_period_end" "date",
  "p_gross_yield_amount" numeric,
...
DECLARE
  ...
  v_crystal_gross_total numeric := 0;
  -- Adjusted period
  v_period_start date := p_period_start;
  v_period_end date := p_period_end;
BEGIN
  ...
  -- FIX 3: For reporting, adjust period_start to GREATEST(month_start, inception_date)
  ...
  -- Count crystallization events in period (for reporting info)
  IF p_purpose = 'reporting' THEN
    SELECT COUNT(*), COALESCE(SUM(gross_yield), 0)
    INTO v_crystals_in_period, v_crystal_gross_total
  ...
  LOOP
    ...
    IF v_gross_share >= v_dust_tolerance THEN
      ...
      IF v_gross_share > v_largest_gross THEN
        ...
        v_largest_fee_pct := v_investor.fee_pct;
        v_largest_ib_rate := v_investor.ib_rate;
      END IF;
  ...
  -- Largest remainder: proportional residual adjustment in preview
  ...
  IF v_residual != 0 AND jsonb_array_length(v_allocations) > 0 THEN
    ...
    v_allocations := jsonb_set(v_allocations,
    ...
    v_allocations := jsonb_set(v_allocations,
      ...
      to_jsonb((v_allocations->v_largest_idx->>'net_yield')::numeric + v_adj_net));
    v_allocations := jsonb_set(v_allocations,
      ARRAY[v_largest_idx::text, 'fee_amount'],
      to_jsonb((v_allocations->v_largest_idx->>'fee_amount')::numeric + v_adj_fee));
    v_allocations := jsonb_set(v_allocations,
      ARRAY[v_largest_idx::text, 'ib_amount'],
      to_jsonb((v_allocations->v_largest_idx->>'ib_amount')::numeric + v_adj_ib));
  ...
  RETURN jsonb_build_object(
    ...
    'calculation_method', 'adb_v4',
    'crystals_in_period', v_crystals_in_period,
    'crystal_gross_total', v_crystal_gross_total,
    'features', ARRAY['time_weighted', 'loss_carryforward', 'ib_fee_exempt', 'ib_commission', 'fees_account_in_adb', 'largest_remainder_proportional', 'satoshi_dust_tolerance', 'fee_hierarchy_3tier', 'reporting_consolidation', 'visibility_scope_control', 'inception_date_period_start']
  );
END;
$$;
