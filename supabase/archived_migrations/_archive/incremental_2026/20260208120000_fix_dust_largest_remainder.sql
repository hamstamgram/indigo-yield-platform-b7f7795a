-- Fix: Eliminate dust from yield distribution using largest remainder method
-- After the allocation loop, any residual (gross - SUM(allocated)) is added to
-- the largest allocation's net yield. This ensures exact conservation:
--   SUM(gross_shares) = gross_yield_amount
--   dust_amount = 0 (always)

-- ============================================================================
-- apply_adb_yield_distribution_v3 - with largest remainder dust fix
-- ============================================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY['time_weighted', 'loss_carryforward', 'fee_credit', 'ib_commission', 'latest_period_reporting', 'ib_positions_in_adb', 'ib_fee_exempt', 'fee_allocations', 'audit_log', 'distribution_date', 'fees_account_in_adb', 'ib_auto_payout', 'largest_remainder_zero_dust']
  );
END;
$$;

-- ============================================================================
-- preview_adb_yield_distribution_v3 - with largest remainder dust fix
-- ============================================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY['time_weighted', 'loss_carryforward', 'ib_fee_exempt', 'ib_commission', 'fees_account_in_adb', 'largest_remainder_zero_dust']
  );
END;
$$;
