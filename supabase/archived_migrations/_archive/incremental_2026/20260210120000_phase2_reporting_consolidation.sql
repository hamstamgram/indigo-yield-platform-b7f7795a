-- Migration: Phase 2 Reporting Consolidation
-- Date: 2026-02-10
--
-- 2.1: Add consolidated_into_id column to yield_distributions
-- 2.2: Create apply_adb_yield_distribution_v4 (replaces v3 for all purposes)
--      - Transaction purpose: standard ADB (same as v3)
--      - Reporting purpose: consolidates mid-month crystallization events into one distribution
-- 2.3: Create preview_adb_yield_distribution_v4
-- 2.4: Fix largest remainder to adjust fee/ib proportionally
-- 2.5: Drop v3 RPCs

-- ============================================================================
-- 2.1: Add consolidated_into_id column
-- Nullable FK linking crystallization distributions to the reporting distribution
-- ============================================================================
ALTER TABLE yield_distributions
  ADD COLUMN IF NOT EXISTS consolidated_into_id uuid REFERENCES yield_distributions(id);

COMMENT ON COLUMN yield_distributions.consolidated_into_id IS
  'When set, this crystallization distribution was consolidated into the referenced reporting distribution';


-- ============================================================================
-- 2.2: apply_adb_yield_distribution_v4
-- Replaces v3. Handles both transaction and reporting purposes.
-- Reporting purpose: consolidates mid-month crystallization events.
-- ============================================================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE fund_id = p_fund_id AND period_end = v_period_end
      ...
      AND is_voided = false
      AND consolidated_into_id IS NULL  -- Allow crystallization dists that were consolidated
  ) THEN
  ...
  RETURN jsonb_build_object(
    ...
    'crystals_consolidated', v_crystals_consolidated,
    'features', ARRAY['time_weighted', 'loss_carryforward', 'fee_credit', 'ib_commission', 'latest_period_reporting', 'ib_positions_in_adb', 'ib_fee_exempt', 'fee_allocations', 'audit_log', 'distribution_date', 'fees_account_in_adb', 'ib_auto_payout', 'largest_remainder_proportional', 'satoshi_dust_tolerance', 'profiles_fee_only', 'admin_recorded_aum', 'fee_ib_overflow_guard', 'purpose_uniqueness', 'reporting_consolidation', 'visibility_scope_control']
  );
END;
$$;
...
-- ============================================================================
-- 2.3: preview_adb_yield_distribution_v4
-- Read-only preview matching v4 apply logic
-- ============================================================================
...
BEGIN
  ...
  IF p_purpose = 'reporting' THEN
    ...
    WHERE fund_id = p_fund_id
      AND effective_date >= p_period_start AND effective_date <= p_period_end
      AND is_voided = false
      AND distribution_type IN ('deposit', 'withdrawal', 'transaction')
      AND consolidated_into_id IS NULL;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'features', ARRAY['time_weighted', 'loss_carryforward', 'ib_fee_exempt', 'ib_commission', 'fees_account_in_adb', 'largest_remainder_proportional', 'satoshi_dust_tolerance', 'profiles_fee_only', 'reporting_consolidation', 'visibility_scope_control']
  );
END;
$$;
...
-- ============================================================================
-- 2.5: Drop v3 RPCs (replaced by v4)
-- ============================================================================

DROP FUNCTION IF EXISTS public.apply_adb_yield_distribution_v3(uuid, date, date, numeric, uuid, aum_purpose, date, numeric);
DROP FUNCTION IF EXISTS public.preview_adb_yield_distribution_v3(uuid, date, date, numeric, text);
