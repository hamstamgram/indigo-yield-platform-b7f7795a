-- Migration: Segmented Yield Distribution V5
-- Date: 2026-02-12
--
-- Replaces ADB time-weighted allocation with segmented proportional model.
-- At month-end, V5 reads crystallization events to split the month into segments.
-- Each segment allocates yield proportionally by balance.
-- One aggregated YIELD tx per investor covers the full month.
--
-- Key changes from V4:
--   - No calc_avg_daily_balance dependency
--   - Segments derived from crystallization metadata (yield_distributions + fund_aum_events)
--   - Running balances track NET yield, fees, and IB between segments
--   - Per-segment fee lookup via get_investor_fee_pct(investor, fund, segment_date)
...
-- ============================================================================
-- 3. UPDATE void_yield_distribution with crystal cascade option
-- ============================================================================
...
BEGIN
  ...
  IF p_void_crystals THEN
    ...
    END LOOP;
  ELSE
    -- Just un-set consolidated_into_id
    UPDATE yield_distributions
  ...
  -- Void investor_yield_events linked to voided transactions
  UPDATE investor_yield_events SET is_voided = true
...
-- ============================================================================
-- 4. DROP V4 FUNCTIONS (clean break)
-- ============================================================================

DROP FUNCTION IF EXISTS apply_adb_yield_distribution_v4(uuid, date, date, numeric, uuid, aum_purpose, date, numeric);
DROP FUNCTION IF EXISTS apply_adb_yield_distribution_v4(uuid, date, date, numeric, uuid, aum_purpose, date, numeric, text);
DROP FUNCTION IF EXISTS preview_adb_yield_distribution_v4(uuid, date, date, numeric, text);
