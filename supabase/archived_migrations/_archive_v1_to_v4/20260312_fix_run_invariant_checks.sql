...
BEGIN
  ...
  -- ========== CORE CHECK 2: fund_aum_matches_positions (OBSOLETE) ==========
  -- In V6, fund_daily_aum is dropped and AUM is dynamically derived from positions. This check is tautological.
  v_violations := '[]'::jsonb;
  v_violation_count := 0;
  v_checks := v_checks || jsonb_build_object('name','fund_aum_matches_positions','category','core','passed',true,'violation_count',0,'violations',v_violations, 'note', 'Obsolete check. AUM is dynamically derived from positions.');
