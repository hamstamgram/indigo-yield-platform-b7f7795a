-- Fix V5 yield distribution: tx_date and IB rate lookup
--
-- 1. Add p_distribution_date parameter (fixes FEE_CREDIT/IB_CREDIT tx_date being
--    forced to period_end instead of the actual distribution date)
-- 2. Replace direct profiles.ib_percentage reads with get_investor_ib_pct() calls
--    (enables per-segment IB rate schedule lookups)

-- Drop old 5-param signature to avoid overload ambiguity
...
BEGIN
  ...
  -- FIX: Use p_distribution_date if provided, otherwise fall back to period_end
  ...
  -- FIX: Use get_investor_ib_pct() instead of profiles.ib_percentage
  -- ============================================================
  ...
  FOR v_seg_idx IN 1..v_seg_count LOOP
    ...
    -- FIX: Use get_investor_ib_pct() for IB rate on new investors joining mid-period
    FOR v_inv IN
    ...
    -- FIX: Use get_investor_ib_pct() per segment for dynamic IB rate
    FOR v_inv IN
