-- FIX: crystallize_yield_before_flow
-- This version matches the production schema (verified via types.ts and baseline)
-- but includes the bug fixes from 20260221:
-- 1. Use <= instead of < for AUM lookup (include same-day)
-- 2. Handle negative yield as 0 instead of throwing an error
...
begin
  ...
  -- FIX 2: Treat negative yield as zero instead of blocking
  ...
  if v_opening_aum > 0 and v_yield_amount > 0 then
    ...
    -- Corrected insert for fund_yield_snapshots
    ...
    -- Corrected insert for yield_distributions
    ...
    loop
      ...
      -- Corrected insert for investor_yield_events
      insert into investor_yield_events (
        fund_id, investor_id, event_date, trigger_type, trigger_transaction_id,
        ...
        p_fund_id, v_investor.investor_id, v_event_date, p_trigger_type,
