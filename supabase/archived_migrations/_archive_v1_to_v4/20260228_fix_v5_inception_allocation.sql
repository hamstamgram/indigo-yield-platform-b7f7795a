-- Migration: Fix V5 inception month allocation (preview + apply)
-- Date: 2026-02-28
--
-- BUG: opening_balance = SUM(tx_date < period_start). For inception months,
-- ALL deposits happen ON period_start, so opening_balance = 0 for everyone.
-- The allocation block requires opening_balance_sum > 0, so it's skipped entirely
-- and the preview/apply returns zero investors.
--
-- FIX: When opening_balance_sum = 0 (inception month), fall back to using
-- deposit-based balances (DEPOSIT + WITHDRAWAL + ADJUSTMENT up to period_end)
-- as the allocation basis. The yield formula stays unchanged.
