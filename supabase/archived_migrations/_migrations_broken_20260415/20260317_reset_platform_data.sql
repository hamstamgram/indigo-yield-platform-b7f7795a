-- reset_platform_data(): Wipes all transactional data while preserving structure.
-- Keeps: funds, profiles, investor_fee_schedule, ib_commission_schedule, user_roles, global_fee_settings, system_config
-- Requires: p_confirm = 'RESET' to execute (safety gate)

CREATE OR REPLACE FUNCTION public.reset_platform_data(p_confirm text DEFAULT '')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
