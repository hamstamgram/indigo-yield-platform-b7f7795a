-- Fix 1: Add missing permissions for ADB yield distribution V3
GRANT EXECUTE ON FUNCTION public.apply_adb_yield_distribution_v3(uuid, date, date, numeric, uuid, aum_purpose) TO authenticated;
GRANT EXECUTE ON FUNCTION public.preview_adb_yield_distribution_v3(uuid, date, date, numeric, text) TO authenticated;

-- Fix 2: Fix format string bug in apply_daily_yield_to_fund_v2 (%.8f is not supported by format())
...
BEGIN
  ...
  IF v_conservation_check > 0.01 THEN
    RETURN jsonb_build_object(
      'success', false, 
      -- FIX: Use %s instead of %.8f
      'error', format('Conservation check failed: gross (%s) != net (%s) + fees (%s), diff: %s',
