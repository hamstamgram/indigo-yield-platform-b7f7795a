-- Fix: Add canonical mutation flag to apply_adb_yield_distribution
-- Issue: CANONICAL_MUTATION_REQUIRED error when applying yield distribution
-- Root cause: Function inserts into yield_distributions without setting app.canonical_rpc

-- ============================================================================
-- 1. Fix the main function to set canonical flag
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Create YIELD transaction for investor (canonical flag already set)
...
Sets app.canonical_rpc flag to bypass mutation triggers.
...
-- ============================================================================
-- 2. Create v3 alias for frontend compatibility
-- ============================================================================
...
Delegates to main function which sets canonical_rpc flag.';

-- ============================================================================
-- 3. Also ensure preview function exists with v3 alias
-- ============================================================================
  ...
  LOOP
    ...
    IF v_gross_share >= v_dust_tolerance THEN
      v_allocations := v_allocations || jsonb_build_object(
        ...
        'investor_name', COALESCE(v_investor.first_name || ' ' || v_investor.last_name, v_investor.investor_email),
        'adb', v_investor.adb,
...
'Preview ADB yield distribution. Returns per-investor breakdown with time-weighted allocations. No database writes.';
