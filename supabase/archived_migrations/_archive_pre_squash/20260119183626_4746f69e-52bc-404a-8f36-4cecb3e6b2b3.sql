-- Fix cleanup function - remove non-existent table reference
CREATE OR REPLACE FUNCTION public.cleanup_test_profiles()
RETURNS TABLE(deleted_profile_id uuid, deleted_email text, reason text)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_profile_id uuid; v_email text; v_test_ids uuid[];
BEGIN
  PERFORM public.set_canonical_rpc(true);
  SELECT ARRAY_AGG(p.id) INTO v_test_ids FROM profiles p
  ...
  IF v_test_ids IS NULL OR array_length(v_test_ids, 1) = 0 THEN RETURN; END IF;
  ...
  FOR v_profile_id, v_email IN SELECT p.id, p.email FROM profiles p WHERE p.id = ANY(v_test_ids)
  LOOP
    DELETE FROM profiles WHERE id = v_profile_id;
    deleted_profile_id := v_profile_id; deleted_email := v_email; reason := 'Test account cleaned'; RETURN NEXT;
  END LOOP;
END; $$;