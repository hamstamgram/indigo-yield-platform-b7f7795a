-- Sync legacy summary columns with canonical totals in ADB yield apply
...
BEGIN
  ...
  -- Calculate current AUM for recorded/previous values
  ...
  INSERT INTO yield_distributions (
    ...
    p_period_start,
    p_period_end,
    v_current_aum + p_gross_yield_amount,
    v_current_aum,
  ...
  FOR v_investor IN
    SELECT
      ...
      COALESCE(
        ...
        (SELECT gs.value::numeric FROM global_fee_settings gs WHERE gs.setting_key = 'default_fee_pct'),
        0
      ) as fee_pct,
      COALESCE(
...
-- Backfill legacy totals for existing applied distributions
UPDATE yield_distributions
SET
  net_yield = COALESCE(net_yield, total_net_amount),
  total_fees = COALESCE(total_fees, total_fee_amount),
  total_ib = COALESCE(total_ib, total_ib_amount)
WHERE status = 'applied'
  AND (net_yield IS NULL OR net_yield = 0)
