-- =============================================================================
-- ROOT CAUSE FIX: Reporting mode AUM handling
-- =============================================================================
--
-- Problem: Reporting yield distributions create spurious "transaction" AUM entries
-- because 4 triggers fire and none check if the transaction purpose is "reporting".
--
-- Root causes fixed:
-- 1. apply function: reporting tx_date defaults to CURRENT_DATE instead of period_end
-- 2. apply function: doesn't suppress AUM sync triggers for reporting distributions
-- 3. sync_aum_on_transaction: doesn't check transaction purpose
-- 4. sync_fund_aum_after_position: doesn't check indigo.aum_synced flag
-- 5. auto_heal_aum_drift: doesn't check indigo.aum_synced flag
-- 6. cascade_void_to_allocations: doesn't void fund_daily_aum when distribution voided
-- =============================================================================


-- ============================================================
-- FIX 1: sync_aum_on_transaction — skip reporting-purpose transactions
-- ============================================================
CREATE OR REPLACE FUNCTION sync_aum_on_transaction()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ============================================================
-- FIX 2: sync_fund_aum_after_position — respect indigo.aum_synced flag
-- ============================================================
CREATE OR REPLACE FUNCTION sync_fund_aum_after_position()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- ============================================================
-- FIX 3: auto_heal_aum_drift — respect indigo.aum_synced flag
-- ============================================================
...
DECLARE
  ...
  v_already_synced text;
BEGIN
  -- FIX: Skip when AUM is being managed by the apply function or transaction trigger
  BEGIN
    v_already_synced := current_setting('indigo.aum_synced', true);
    IF v_already_synced = 'true' THEN
      RETURN NEW;
  ...
  IF v_difference > v_tolerance AND v_recorded_aum IS NOT NULL THEN
    ...
    -- Only heal transaction-purpose entries (never overwrite reporting entries)
    UPDATE fund_daily_aum
    ...
    WHERE fund_id = NEW.fund_id
      ...
      AND aum_date = (
        ...
        WHERE fund_id = NEW.fund_id AND is_voided = false AND purpose = 'transaction'
...
-- ============================================================
-- FIX 4: cascade_void_to_allocations — also void fund_daily_aum
-- ============================================================
...
AS $$
BEGIN
  -- When yield_distribution is voided, cascade to related records
  IF NEW.status = 'voided' AND (OLD.status IS NULL OR OLD.status != 'voided') THEN
    ...
    -- FIX: Void the associated fund_daily_aum entry created by this distribution
    UPDATE fund_daily_aum
    SET is_voided = true,
        voided_at = now(),
        voided_by = NEW.voided_by,
...
-- ============================================================
-- FIX 5: apply_adb_yield_distribution_v3 — reporting mode fixes
--   a) Default tx_date to period_end in reporting mode
--   b) Set indigo.aum_synced before booking to suppress triggers
-- ============================================================
...
BEGIN
  ...
  -- Reporting mode: derive period, set tx_date to period end, suppress AUM triggers
  IF p_purpose = 'reporting'::aum_purpose THEN
    ...
    -- FIX (root cause 1): Default tx_date to period_end for historical accuracy
    v_tx_date := COALESCE(p_distribution_date, v_period_end);

    -- FIX (root cause 2): Suppress all AUM sync triggers — we handle AUM ourselves
    PERFORM set_config('indigo.aum_synced', 'true', true);

    -- Derive yield from existing transactions only when user provides 0 or NULL
    ...
    END IF;
  ELSE
    -- Transaction mode: use distribution_date or today
  ...
  -- Calculate current AUM (investor + IB positions only)
  ...
  -- Calculate total ADB (investor + IB positions only)
  ...
  FOR v_investor IN
    SELECT
      ...
      CASE WHEN p.account_type = 'ib' THEN 0
        ELSE COALESCE(
  ...
  -- AUM record: UPDATE existing or INSERT new (handles stale entries from voided distributions)
  UPDATE fund_daily_aum
