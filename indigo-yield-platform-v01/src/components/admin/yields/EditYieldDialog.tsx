...
export function EditYieldDialog({
  ...
  useEffect(() => {
    if (record && open) {
      setNewAum((record.total_aum ?? 0).toString());
      setReason("");
    }
  }, [record, open]);
  ...
  const hasChanged = record && parsedNewAum !== (record.total_aum ?? 0);
  ...
  const delta = isValidAum ? parsedNewAum - (record.total_aum ?? 0) : 0;
  const deltaPercent = (record.total_aum ?? 0) > 0 ? (delta / (record.total_aum ?? 0)) * 100 : 0;
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-center">
              <div className="bg-muted/50 rounded-lg p-3 text-center">
                ...
                <p className="font-mono font-medium">
                  {(record.total_aum ?? 0).toLocaleString(undefined, { maximumFractionDigits: 8 })}
                </p>
              </div>
            ...
            <div className={`rounded-lg p-3 ${delta >= 0 ? "bg-green-50 dark:bg-green-900/20" : "bg-red-50 dark:bg-red-900/20"}`}>
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-mono font-medium ${delta >= 0 ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}>
                  ...
                  {" "}
                  ({deltaPercent >= 0 ? "+" : ""}{(deltaPercent ?? 0).toFixed(2)}%)
                </span>
              </div>
            </div>
          )}
