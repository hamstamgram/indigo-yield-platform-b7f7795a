...
export function YieldEventsTable({ initialFundId, className }: YieldEventsTableProps) {
  ...
  const [visibilityFilter, setVisibilityFilter] = useState<"all" | "admin_only" | "investor_visible">("all");
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((e) =>
        e.investor_id.toLowerCase().includes(search)
      );
    }
  ...
  const getTriggerBadge = (trigger: string) => {
    switch (trigger) {
      case "deposit":
        return <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200">Deposit</Badge>;
      case "withdrawal":
        return <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">Withdrawal</Badge>;
      case "month_end":
        return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Month End</Badge>;
      case "manual":
  ...
  const formatPercent = (value: number | undefined | null) => {
    ...
    return `${(safeValue * 100).toFixed(4)}%`;
  };
      ...
      <CardContent className="space-y-4">
        ...
        {filteredEvents.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">Events will appear after deposits, withdrawals, or month-end processing</p>
          </div>
        ) : (
              ...
              <TableBody>
                {filteredEvents.slice(0, 100).map((event) => (
                  <TableRow
                    key={event.id}
                    className={cn(
                      event.visibility_scope === "admin_only" && "bg-amber-50/50"
                    )}
                  >
                    ...
                    <TableCell className="text-right font-mono">
                      {formatPercent(event.investor_share_pct)}
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {formatPercent(event.fund_yield_pct)}
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {formatNumber(event.gross_yield_amount, 6)}
                    </TableCell>
                    <TableCell className="text-right font-mono text-muted-foreground">
                      -{formatNumber(event.fee_amount, 6)}
                    </TableCell>
                    <TableCell
                      className={cn(
                    ...
                    >
                      {formatNumber(event.net_yield_amount, 6)}
                    </TableCell>
        ...
        {filteredEvents.length > 0 && (
          <div className="flex items-center justify-between text-sm text-muted-foreground pt-2 border-t">
            ...
            <span className="font-mono">
              Total Net Yield: {formatNumber(stats.totalYield, 6)}
            </span>
          </div>
        )}
      </CardContent>
