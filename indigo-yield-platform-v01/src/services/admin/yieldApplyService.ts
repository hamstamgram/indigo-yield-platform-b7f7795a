...
/**
 * Pre-yield validation result interface
 */
interface PreYieldValidation {
  is_valid: boolean;
  recorded_aum: number;
...
/**
 * Validate AUM reconciliation before applying yield
 * Prevents yield distribution when position data is inconsistent
 */
export async function validateBeforeYield(
  fundId: string,
  tolerancePercentage: number = 1.0
): Promise<PreYieldValidation> {
  const { data, error } = await callRPC("validate_pre_yield_aum", {
    p_fund_id: fundId,
    p_tolerance_percentage: tolerancePercentage,
  });

  if (error) {
    logError("validateBeforeYield", error, { fundId });
    throw new Error(`Failed to validate AUM: ${error.message}`);
  }

  return data as unknown as PreYieldValidation;
}

/**
 * Check AUM reconciliation status for a fund
 */
export async function checkAumReconciliation(
  ...
  const { data, error } = await callRPC("check_aum_reconciliation", {
    p_fund_id: fundId,
    p_tolerance_pct: tolerance,
  });

  if (error) {
    logError("checkAumReconciliation", error, { fundId });
    throw new Error(`Failed to check AUM reconciliation: ${error.message}`);
  }

  const result = data as any;
  return {
    is_reconciled: result.is_reconciled,
    recorded_aum: Number(result.recorded_aum),
    calculated_aum: Number(result.calculated_aum),
...
export async function applyYieldDistribution(
  ...
  // Pre-yield AUM validation gate (unless explicitly skipped)
  if (!skipValidation && purpose === "transaction") {
    const validation = await validateBeforeYield(fundId, 1.0);

    if (!validation.is_valid) {
      const errorMessages = validation.errors.join("; ");
      logError("applyYieldDistribution.validation", new Error(errorMessages), {
        fundId,
        recorded_aum: validation.recorded_aum,
        calculated_aum: validation.calculated_aum,
        discrepancy_percentage: validation.discrepancy_percentage,
      });
      throw new Error(
        `Cannot apply yield: AUM validation failed. ${errorMessages}`
      );
    }

    // Log warnings if any
    if (validation.warnings.length > 0) {
      logWarn("applyYieldDistribution.validationWarnings", {
        fundId,
