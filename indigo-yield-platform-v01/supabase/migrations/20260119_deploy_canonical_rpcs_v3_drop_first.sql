-- ============================================================================
-- Migration: Deploy Canonical RPC Functions V3 (with DROP FUNCTION first)
-- Created: 2026-01-19
-- Purpose: DROP existing functions then recreate with correct return types
-- ============================================================================

-- ============================================================================
-- STEP 1: DROP EXISTING FUNCTIONS (to fix return type conflicts)
-- ============================================================================
...
DROP FUNCTION IF EXISTS public.set_fund_daily_aum(UUID, DATE, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS public.batch_reconcile_all_positions();
DROP FUNCTION IF EXISTS public.sync_all_fund_aum(DATE);
DROP TRIGGER IF EXISTS trg_auto_recompute_position ON transactions_v2;
DROP FUNCTION IF EXISTS public.trg_auto_recompute_position_fn();

-- ============================================================================
-- STEP 2: Create position_correction_log table if not exists
-- ============================================================================
...
-- ============================================================================
CREATE FUNCTION public.recompute_investor_position(
  p_investor_id UUID,
  p_fund_id UUID
)
RETURNS JSONB
...
BEGIN
  ...
  IF v_position_exists THEN
    ...
    RETURN jsonb_build_object(
      'success', true, 'investor_id', p_investor_id, 'fund_id', p_fund_id,
      'old_value', v_old_value, 'new_value', v_net_balance,
      'variance', v_old_value - v_net_balance, 'operation', 'update'
    );
  ELSE
...
-- ============================================================================
-- IMMEDIATE RECONCILIATION - Run batch reconciliation
-- ============================================================================
